...
const CatalogGrid = ({
  ...
  const totalItems = filteredCatalogData.reduce((acc, cat) => acc + cat.items.length, 0);
  ...
  const handleDishClick = (item) => {
    ...
    if (hasVariants || hasAddons) {
      setSelectedCustomizeDish(item);
      if (hasVariants && Array.isArray(item.variants) && item.variants.length > 0) {
        const firstAvailable = item.variants.find((v) => v.is_available !== false) || item.variants[0];
  ...
  // Helper to get added quantity for a item (sums quantities of all customized versions too)
  const getAddedQty = (dishName) => {
    return orderItems.filter((oi) => oi.item_name === dishName).reduce((sum, oi) => sum + oi.quantity, 0);
  };
      ...
      <div className="pos-catalog-area">
        <div className="pos-catalog-filters">
          ...
          <div className="pos-search-wrap">
            ...
            <Form.Control className="pos-search-input" placeholder="Search items..." value={searchText} onChange={(e) => setSearchText(e.target.value)} />
            {searchText && (
              ...
              filteredCatalogData.map((category) => (
                <div key={category._id} className="mb-4">
                  <div className="pos-section-header">
                    ...
                    <span className="pos-section-count">{category.items.length}</span>
                  </div>
                  <div className="pos-grid-5">
                    {category.items.map((item) => {
                      const addedQty = getAddedQty(item.item_name);
                      return (
                        <div className="pos-grid-item" key={item._id}>
                          <div
                            className="pos-catalog-card"
                            onClick={() => handleDishClick(item)}
                            role="button"
                            tabIndex={0}
                            onKeyDown={(e) => e.key === 'Enter' && handleDishClick(item)}
                          >
                            {/* Qty Badge */}
                            {addedQty > 0 && <div className="pos-added-badge">{addedQty} Added</div>}

                            {/* Type Indicator */}
                            <div className={`pos-type-dot ${(item.type || 'veg') === 'veg' ? 'veg-dot' : (item.type || 'veg') === 'egg' ? 'egg-dot' : 'nonveg-dot'}`} />

                            {/* Image */}
                            <div className="pos-catalog-img-wrap">
                              {item.item_img ? (
                                <img src={`${uploadDir}/${item.item_img}`} alt={item.item_name} className="pos-catalog-img" />
                              ) : (
                    ...
                    <div className="pos-grid-item" key={charge._id}>
                      ...
                      >
                        ...
                        <div className="pos-catalog-details">
                          ...
                          <div className="pos-item-price">₹{charge.price}</div>
                          <div className="pos-add-hint">+ Add</div>
                        </div>
                      </div>
                    </div>
        ...
        <Modal show={showCustomizeModal} onHide={() => setShowCustomizeModal(false)} centered className="customize-modal" size="md">
          ...
          <Modal.Header closeButton className="border-0 pb-0">
            <div>
              ...
              <div className="text-dark fw-bold" style={{ fontSize: '1.05rem', marginTop: '3px' }}>{selectedCustomizeDish.item_name}</div>
            </div>
          </Modal.Header>
          ...
          <Modal.Footer className="border-0">
            ...
            <Button
              ...
              onClick={() => {
                const basePrice = selectedVariant ? Number(selectedVariant.price) : Number(selectedCustomizeDish.item_price);
            ...
            >
              ...
              Add to Cart • ₹
              {(
                (selectedVariant ? Number(selectedVariant.price) : Number(selectedCustomizeDish.item_price)) +
