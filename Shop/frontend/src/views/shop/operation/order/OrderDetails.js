...
const OrderDetails = () => {
  ...
  const description = 'Detailed view of a specific sale including customer, billing, and sales items.';

  const breadcrumbs = [
    { to: '', text: 'Home' },
    { to: 'operations', text: 'Operations' },
    { to: 'operations/sales-history', text: 'Sales History' },
    { to: '', title: 'Sales Details' },
  ];
  ...
  const sendWhatsAppMessage = async (phoneVal) => {
    setSharing(true);
    try {
      const restaurantName = restaurantInfo?.name || 'Shop';
      ...
      order.order_items.forEach(item => {
        message += `- ${item.quantity} x ${item.item_name} - ₹${(item.item_price * item.quantity).toFixed(2)}\n`;
      });
          ...
          <Card className="h-100 border-0 shadow-sm" style={{ borderRadius: '1.25rem' }}>
            ...
            <Card.Body>
              ...
              <Row className="g-3 mb-4">
                <Col xs={6}>
                  <div className="text-small text-muted mb-1">SALES NUMBER</div>
                  <div className="h6 mb-0">{order.order_no || order.id || '-'}</div>
                </Col>
                <Col xs={6}>
                  <div className="text-small text-muted mb-1">SALES DATE</div>
          ...
          <Card className="h-100 border-0 shadow-sm" style={{ borderRadius: '1.25rem' }}>
            ...
            <Card.Body>
                ...
                <Table className="align-middle" hover>
                  <thead className="table-light">
                    <tr>
                      ...
                      <th scope="col" className="text-muted text-small text-uppercase">Item</th>
                  ...
                  <tbody>
                    {order.order_items?.map((item, index) => (
                      <tr key={`${item.item_name}-${index}`}>
                        ...
                        <td className="text-end">₹ {parseFloat(item.item_price).toFixed(2)}</td>
                        <td className="text-end fw-medium text-primary">₹ {(parseFloat(item.item_price) * parseFloat(item.quantity)).toFixed(2)}</td>
              ...
              <div className="d-md-none">
                {order.order_items?.map((item, index) => (
                  <div key={`${item.item_name}-${index}`} className="border-bottom py-3 last-child-border-0">
                    <div className="d-flex justify-content-between align-items-start mb-2">
                      <div className="d-flex align-items-center">
                        ...
                        <div>
                          ...
                          <div className="text-muted xsmall">Qty: {item.quantity} × ₹{parseFloat(item.item_price).toFixed(2)}</div>
                        </div>
                      </div>
                      <Badge bg={
                        item.status === 'Served' || item.status === 'Completed' || item.status === 'Paid' ? 'success' :
                    ...
                    <div className="d-flex justify-content-between align-items-center ps-7">
                      ...
                      <div className="fw-bold text-primary">₹ {(parseFloat(item.item_price) * parseFloat(item.quantity)).toFixed(2)}</div>
                    </div>
                  </div>
                ))}
              </div>
