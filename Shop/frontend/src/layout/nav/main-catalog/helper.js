...
// Decides which type of catalog to add based on the parameters or the current window size.
// placementStatus:
...
export const checkPlacement = ({ placement, breakpoints }) => {
  ...
  if (placement === MENU_PLACEMENT.Horizontal) {
    // Adding vertical catalog for mobile
    ...
    // Adding horizontal catalog for desktop
    return {
      status: 2,
      placementHtmlData: MENU_PLACEMENT.Horizontal,
  ...
  if (placement === MENU_PLACEMENT.Vertical) {
    ...
    // Adding vertical catalog for desktop
    return {
      status: 4,
...
// Decides which type of catalog behaviour to init based on the parameters or the current window size.
//  behaviourStatus:
...
// Using popperjs to align user,language and notification dropdowns in the catalog
