...
const clearMenuItem = (catalogItem) => {
  const item = {};
  ['path', 'label', 'icon', 'isExternal', 'subs', 'mega', 'megaParent'].forEach((key) => {
    if (catalogItem[key]) item[key] = catalogItem[key];
  });
  return item;
};
const clearFlattedMenuItem = (catalogItem) => {
  const item = {};
            ...
            if (tempItem.roles) {
              if (!sub.roles) controlledSub.roles = tempItem.roles;
                ...
                if (controlledSub.roles && controlledSub.roles.length === 0) {
                  controlledSub.inaccessible = true;
                  console.log(
                    `This catalog item(${controlledSub.path}) is inaccessible. Please check the roles you defined in the hierarchical structure.`,
                    controlledSub
                  );
                }
              }
...
export const convertToSearchItems = ({ data = [], authGuardActive = IS_AUTH_GUARD_ACTIVE, isLogin = false, userRole = null }) => {
  ...
  return () => {
    const itemMapper = (item) => {
      ...
      if (Object.keys(tempItem).length > 0 && tempItem.label) catalogItems.push(clearFlattedMenuItem(tempItem));
