...
const recalculateOrderTotals = (orderInfo) => {
  ...
  orderInfo.order_items = orderInfo.order_items.map((item) => {
    let basePrice = item.item_price; // Default fallback
    ...
    item.item_price = finalDishPrice;
...
const assignCountersToItems = async (userId, orderItems) => {
  ...
  // Build item_name → counter map
  const dishCounterMap = {};
  menus.forEach(catalog => {
    const counter = catalog.counter || "Default";
    catalog.items.forEach(item => {
      dishCounterMap[item.item_name] = counter;
    });
  });
  ...
  return orderItems.map(item => ({
    ...item,
    counter: item.counter || dishCounterMap[item.item_name] || "Default"
...
const addKotsFlag = async (userId, orderItems) => {
  ...
  // Build item_name → hide_on_kot map
  const hideOnKOTsDishSet = {};
  menus.forEach(catalog => {
    const hideOnKot = catalog.hide_on_kot || false;
    catalog.items.forEach(item => {
      hideOnKOTsDishSet[item.item_name] = hideOnKot;
    });
  });
  ...
  return orderItems.map(item => ({
    ...item,
    hide_on_kot: hideOnKOTsDishSet[item.item_name] || item.hide_on_kot || false
  }));
};
...
const dineInController = async (req, res) => {
  try {
    ...
    // assign counters to items (only if catalog exists, safe fallback)
