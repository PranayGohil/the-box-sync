...
exports.addToCart = async (req, res) => {
    try {
        ...
        const catalog = await Catalog.findOne({ "items._id": item_id });
        if (!catalog) {
            return res.status(404).json({
                success: false,
        ...
        const product = catalog.items.find((d) => d._id.toString() === item_id);
        if (!product) {
            return res.status(404).json({
                success: false,
        ...
        const existingCartItem = customer.cart.find((item) => item.item_id.toString() === item_id);
        if (existingCartItem) {
...
exports.getCart = async (req, res) => {
    try {
        ...
        const customer = await WebCustomer.findById(id).populate('cart.item_id');
        if (!customer) {
            return res.status(404).json({
                success: false,
                message: 'Customer not found',
...
exports.removeFromCart = async (req, res) => {
    try {
        ...
        const productIndex = customer.cart.findIndex((item) => item.item_id.toString() === item_id);
        if (productIndex === -1) {
            return res.status(404).json({
                success: false,
