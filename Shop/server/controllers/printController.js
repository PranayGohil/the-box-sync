    ...
    const itemsHTML = order.order_items.map(item => {
      ...
      return `
        <tr>
          ...
          <td style="padding: 2px 1px; text-align: center; vertical-align: top; font-size: 5px; color: #000;">₹${item.item_price}</td>
          <td style="padding: 2px 1px; text-align: right; vertical-align: top; font-size: 5px; color: #000;">₹${(item.item_price * item.quantity).toFixed(2)}</td>
        </tr>
      `;
    }).join('');
