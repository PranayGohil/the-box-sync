...
const updateDishStatus = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Error updating item status.", error });
  }
};
...
const updateAllDishStatus = async (req, res) => {
  try {
    ...
    // If you only want to update items that are currently 'Preparing', use arrayFilters.
