  ...
  try {
    ...
    if (!settings) {
      ...
      return res.json({
        ...
        contact_details: "Whether you're booking a table, asking about our catalog, or just want to say hello — reach out and we'll respond promptly.",
      });
    }
...
exports.updateWebsiteSettings = async (req, res) => {
  try {
    ...
    // Parse featured_item_ids only if it exists and is a valid JSON string
    let parsedFeaturedDishes = [];
    if (featured_item_ids) {
      try {
        parsedFeaturedDishes = JSON.parse(featured_item_ids);
      } catch (e) {
        return res
          .status(400)
          .json({ error: "Invalid featured_item_ids format" });
      }
    }
    ...
    const updated = await Website.findOneAndUpdate(
      { user_id: userId },
      {
        ...
        contact_email,
        contact_phone,
        featured_item_ids: parsedFeaturedDishes,
        logo,
        hero_title,
        hero_subtitle,
        hero_details,
...
exports.getAllDishes = async (req, res) => {
  try {
    ...
    const featuredIds = websiteSettings?.featured_item_ids || [];

    const menus = await Catalog.find({ user_id: userId });

    const categorized = menus.map((catalog) => {
      const itemsWithFlag = catalog.items.map((item) => ({
        ...item._doc,
        _id: item._id.toString(),
        is_featured: featuredIds.includes(item._id.toString()),
      }));
...
exports.getWebsiteSettingsByCode = async (req, res) => {
  try {
    ...
    const response = {
      ...
      contact_details: settings?.contact_details || "Whether you're booking a table, asking about our catalog, or just want to say hello — reach out and we'll respond promptly.",
      hero_title: settings?.hero_title || "Welcome to Our Restaurant",
      hero_subtitle: settings?.hero_subtitle || "Delicious. Authentic. Fresh.",
      hero_details: settings?.hero_details || "Experience extraordinary flavors where every item tells a story.",
...
// ✅ 2. Public Featured Items by restaurant_code
exports.getFeaturedDishesByCode = async (req, res) => {
  try {
    ...
    const menus = await Catalog.find({ user_id: user._id });

    const categorized = menus
      .map((catalog) => {
        const filtered = catalog.items.filter((item) =>
          featuredIds.includes(item._id.toString())
        );
        return {
          category: catalog.category,
          items: filtered,
    ...
    res.status(500).json({ error: "Server error while fetching featured items" });
  }
};
...
// ✅ 4. Public Catalog by restaurant_code
