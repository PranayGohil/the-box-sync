  ...
  try {
    ...
    const pipeline = [
      ...
      },
      {
        $addFields: {
          normalizedDishName: {
            $toLower: { $trim: { input: "$order_items.item_name" } },
          },
        },
      },
      {
        $lookup: {
          ...
          pipeline: [
            ...
            { $unwind: "$items" },
            {
              $addFields: {
                normalizedMenuDish: {
                  $toLower: { $trim: { input: "$items.item_name" } },
                },
              },
            },
            {
    ...
    pipeline.push(
      {
        $group: {
          ...
          dishName: { $first: "$order_items.item_name" },
          totalQuantity: { $sum: "$order_items.quantity" },
          totalRevenue: {
            $sum: {
              $multiply: ["$order_items.quantity", "$order_items.item_price"],
            },
          },
          orderCount: { $sum: 1 },
          category: { $first: "$menuDetail.category" },
          mealType: { $first: "$menuDetail.type" },
          isSpecial: { $first: "$menuDetail.is_special" },
          avgPrice: { $avg: "$order_items.item_price" },
          dishImg: { $first: "$menuDetail.item_img" },
        },
      },
      {
        $project: {
...
const getLowPerformingDishes = async (req, res) => {
  try {
    ...
    const allDishes = await Catalog.aggregate([
      { $match: { user_id: restaurantId } },
      { $unwind: "$items" },
      {
        $match: {
      ...
      {
        $project: {
          dishName: "$items.item_name",
          category: "$category",
          price: "$items.item_price",
          isSpecial: "$items.is_special",
        },
      },
    ]);
    ...
    const orderedDishes = await Order.aggregate([
      ...
      {
        $group: {
          _id: { $toLower: { $trim: { input: "$order_items.item_name" } } },
          orderCount: { $sum: "$order_items.quantity" },
        },
      },
    ]);

    // Create a map of ordered items
    const orderedMap = {};
    orderedDishes.forEach((item) => {
      orderedMap[item._id] = item.orderCount;
    });

    // Find low performing items
    const lowPerforming = allDishes
      .map((item) => {
        const normalizedName = item.dishName.toLowerCase().trim();
      ...
      .filter((item) => item.orderCount < parseInt(threshold))
...
// ============================================
// INVENTORY & CATALOG PERFORMANCE REPORT
// ============================================
...
const getMenuPerformanceReport = async (req, res) => {
  try {
    ...
    const mealTypePerformance = await Order.aggregate([
      ...
      {
        $addFields: {
          mealType: { $arrayElemAt: ["$menuInfo.type", 0] },
        },
      },
      {
        $group: {
