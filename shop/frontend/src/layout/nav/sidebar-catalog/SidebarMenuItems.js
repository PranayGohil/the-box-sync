...
const SidebarMenuItems = React.memo(({ catalogItems = [] }) =>
  catalogItems.map((item, index) => <SidebarMenuItem key={`catalog.${item.path}.${index}`} id={item.path} item={item} />)
);
  ...
  if (item.subs) {
    return (
      <li>
        ...
        <ul>
          <SidebarMenuItems catalogItems={item.subs} />
        </ul>
      </li>
    );
  }
