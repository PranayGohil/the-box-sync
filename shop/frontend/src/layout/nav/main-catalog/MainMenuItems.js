...
import { menuChangeCollapseAll } from './catalogSlice';
...
const MainMenuItem = memo(({ item, id, isSubItem = false, menuPlacement = DEFAULT_SETTINGS.MENU_PLACEMENT }) => {
  ...
  const { collapseAll } = useSelector((state) => state.catalog);
  ...
  if (item.subs && menuPlacement === MENU_PLACEMENT.Horizontal && !item.megaParent) {
    return (
      <Dropdown as="li" key={id} onToggle={onToggleItem} className={classNames({ mega: item.mega })} show={horizontalDropdownIsOpen}>
        ...
        >
          <MainMenuItems catalogItems={item.subs} menuPlacement={menuPlacement} isSubItem />
        </Dropdown.Menu>
      </Dropdown>
    );
  }
...
const MainMenuItems = React.memo(({ catalogItems = [], menuPlacement = DEFAULT_SETTINGS.MENU_PLACEMENT, isSubItem = false }) =>
  catalogItems.map((item, index) => (
    <MainMenuItem key={`catalog.${item.path}.${index}`} id={item.path} item={item} menuPlacement={menuPlacement} isSubItem={isSubItem} />
  ))
);
