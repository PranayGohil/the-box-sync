  ...
  return (
    <Link to="/settings/profile" className="user-container d-flex">
      <div className="d-flex user position-relative align-items-center">
        {isWideLogo ? (
          <img
            alt={currentUser?.name || 'Shop'}
            src={logoUrl}
            style={{ height: '32px', width: 'auto', maxWidth: '140px', objectFit: 'contain' }}
          />
        ) : (
          <>
            <img className="profile" style={{ objectFit: 'cover' }} alt={currentUser?.name || 'Shop'} src={logoUrl} />
            <div className="name">{currentUser?.name || 'Shop'}</div>
          </>
        )}
      </div>
    </Link>
