...
const ViewKots = () => {
  ...
  const description = 'View and manage all KOT orders with item status updates';
  ...
  const updateDishStatus = async (orderId, dishId, status = 'Completed') => {
    try {
      setUpdatingDishId(dishId);
      await axios.put(
        `${process.env.REACT_APP_API}/kot/item/update-status`,
        { orderSource: 'QSR', orderId, dishId, status },
        {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        }
      );
      toast.success(status === 'Completed' ? 'Item completed!' : 'Item updated!');
      fetchOrderData();
    } catch (err) {
  ...
  const handleDishToggle = (kot, item) => {
    if (item.status === 'Completed') {
      updateDishStatus(kot._id, item._id, 'Preparing');
      return;
    }
    ...
    if (pendingItems.length === 1 && pendingItems[0]._id === item._id) {
      ...
      updateDishStatus(kot._id, item._id, 'Completed');
    }
  };
  ...
  const updateAllDishStatus = async (orderId) => {
    try {
      ...
      await axios.put(
        `${process.env.REACT_APP_API}/kot/item/update-all-status`,
  ...
  return (
    <div className="container-fluid pb-5 mb-5">
      ...
      {/* Header Section aligned with ManageCatalog */}
