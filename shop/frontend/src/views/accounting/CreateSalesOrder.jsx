...
import { createSalesOrder } from '../../api/accounting';
import { getCatalogData } from '../../api/orderService';

const breadcrumbs = [
  { to: '', text: 'Home' },
  { to: 'accounting', text: 'Accounting & Billing' },
  { to: 'accounting/create-sales-order', text: 'Sales Order' },
];
...
const styles = `
  ...
  .so-field .form-control:focus,
  .so-field .form-select:focus {
  ...
  .so-line-card:hover { box-shadow: 0 4px 16px rgba(35,179,244,0.08); }
  .so-line-num {
  ...
  .so-summary-label { color: #64748b; }
  .so-summary-val { font-weight: 700; color: #334155; }
  .so-grand-row {
    display: flex;
    justify-content: space-between;
    align-items: center;
  ...
  .so-remove-btn:hover { background: #ef4444; color: #fff; }
  .so-item-total {
  ...
  @media (max-width: 575px) {
    .so-section-card .card-body { padding: 16px !important; }
  }
`;
...
const CreateSalesOrder = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    deliveryDate: '',
    notes: '',
    termsAndConditions: 'Subject to availability of items.'
  });
  ...
  return (
    <>
      ...
      <div className="qsr-page-title-container">
        <h1 className="qsr-page-title">Create Sales Order</h1>
        <BreadcrumbList items={breadcrumbs} />
      </div>

      <Form onSubmit={handleSubmit}>
        {/* Section 1: Customer Details */}
        <Card className="so-section-card">
          <Card.Body className="p-4">
            <div className="so-section-title">
              <CsLineIcons icon="user" size="15" /> Customer Details &amp; Schedule
            </div>
            <Row className="g-3">
              <Col xs={12} md={6}>
              ...
              <Col xs={6} md={4}>
                <div className="so-field">
                  ...
                  <Form.Control type="date" name="deliveryDate" value={formData.deliveryDate} onChange={handleFormChange} />
                </div>
              </Col>
              <Col xs={12} md={4}>
                <div className="so-field">
                  <Form.Label>Address</Form.Label>
                  <Form.Control type="text" name="address" value={formData.customerDetails.address} onChange={handleCustomerChange} placeholder="Delivery address" />
                </div>
              </Col>
            </Row>
          </Card.Body>
        ...
        <Card className="so-section-card">
          <Card.Body className="p-4">
            ...
            {items.map((item, index) => (
              <div className="so-line-card" key={index}>
                <div className="d-flex align-items-center gap-2 mb-3">
                  <div className="so-line-num">{index + 1}</div>
                  <div className="flex-grow-1 so-field mb-0">
                    <CreatableSelect
                      isClearable
                  ...
                  {items.length > 1 && (
                    <button type="button" className="so-remove-btn" onClick={() => removeItem(index)}>×</button>
                  )}
                </div>
                ...
                <div className="d-flex justify-content-between align-items-center mt-3">
                  ...
                  <div className="so-item-total">₹{item.totalAmount.toFixed(2)}</div>
                </div>
              </div>
            ))}
            ...
            <Row className="justify-content-end">
              <Col xs={12} md={6} lg={5}>
                <div className="so-summary-row">
                  ...
                  <span className="so-summary-val">₹{summary.subTotal.toFixed(2)}</span>
                </div>
                {summary.totalCGST > 0 && (
                  <div className="so-summary-row">
                    <span className="so-summary-label">Total CGST</span>
                    <span className="so-summary-val">₹{summary.totalCGST.toFixed(2)}</span>
                  </div>
                )}
                {summary.totalSGST > 0 && (
                  <div className="so-summary-row">
                    <span className="so-summary-label">Total SGST</span>
                    <span className="so-summary-val">₹{summary.totalSGST.toFixed(2)}</span>
                  </div>
                )}
                <div className="so-grand-row">
        ...
        <div className="d-flex flex-column flex-sm-row justify-content-end gap-2 mb-5">
          ...
          <Button type="submit" variant="primary" className="rounded-pill px-5" style={{ fontWeight: 700 }}>
            <CsLineIcons icon="save" size="14" className="me-1" /> Save Sales Order
          </Button>
        </div>
      </Form>
    </>
