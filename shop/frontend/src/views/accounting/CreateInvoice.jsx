...
const CreateInvoice = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    tdsDetails: { isTDSDeducted: false, tdsPercentage: 0, tdsAmount: 0, tdsType: 'NONE' },
    notes: '',
  ...
  const handleItemChange = (index, field, value) => {
    ...
    // Simple assumption: If inter-state (e.g. state is different), it would be IGST. We'll default to CGST/SGST split for now.
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      items,
      summary,
      amountDue: summary.grandTotal - Number(formData.tdsDetails.tdsAmount)
    };

    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/api/accounting/invoices', payload, {
  ...
  return (
    <>
      <div className="page-title-container mb-3">
        <h1 className="mb-2 pb-0 display-4">Create GST Invoice</h1>
      </div>
      ...
      <Form onSubmit={handleSubmit}>
          ...
          <Card.Body>
            ...
            <Row>
              ...
              <Col md={6} className="mb-3">
                ...
                <Form.Control type="text" name="gstin" value={formData.customerDetails.gstin} onChange={handleCustomerChange} />
              </Col>
              <Col md={6} className="mb-3">
              ...
              <tbody>
                {items.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td><Form.Control type="text" value={item.hsnCode} onChange={(e) => handleItemChange(index, 'hsnCode', e.target.value)} /></td>
        ...
        <Button variant="primary" type="submit" className="mb-5">Save Invoice</Button>
      </Form>
    </>
  );
};
