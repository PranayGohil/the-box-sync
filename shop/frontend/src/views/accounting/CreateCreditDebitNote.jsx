...
import { createNote, getInvoices } from '../../api/accounting';
import { getCatalogData } from '../../api/orderService';

const breadcrumbs = [
  { to: '', text: 'Home' },
  { to: 'accounting', text: 'Accounting & Billing' },
  { to: 'accounting/create-note', text: 'Credit / Debit Note' },
];
...
const styles = `
  ...
  .note-field .form-control:focus,
  .note-field .form-select:focus {
  ...
  .note-line-card:hover { box-shadow: 0 4px 16px rgba(35,179,244,0.08); }
  .note-line-num {
  ...
  .note-summary-row:last-child { border-bottom: none; }
  .note-summary-label { color: #64748b; }
  .note-summary-val { font-weight: 700; color: #334155; }
  .note-grand-row {
    display: flex;
    justify-content: space-between;
    align-items: center;
  ...
  .note-remove-btn:hover { background: #ef4444; color: #fff; }
  .note-item-total {
  ...
  @media (max-width: 575px) {
    .note-section-card .card-body { padding: 16px !important; }
  }
`;
...
const CreateCreditDebitNote = () => {
  const history = useHistory();
  const location = useLocation();
  const shopId = localStorage.getItem('shopId');

  const [formData, setFormData] = useState({
    noteNumber: `CN-DN-${Date.now()}`,
    noteType: 'CREDIT',
    invoiceId: '',
    reason: 'Sales Return',
    partyDetails: { name: '', gstin: '', address: '', state: '', phone: '', email: '' },
    notes: ''
  });
  ...
  const handleFormChange = (e) => {
    ...
    setFormData(prev => {
      ...
      if (name === 'noteType') {
        updated.reason = value === 'CREDIT' ? 'Sales Return' : 'Purchase Return';
        updated.noteNumber = `${value === 'CREDIT' ? 'CN' : 'DN'}-${Date.now()}`;
      }
      return updated;
    });
  };
  ...
  const handlePartyChange = (e) => {
    setFormData({ ...formData, partyDetails: { ...formData.partyDetails, [e.target.name]: e.target.value } });
  };

  const handleInvoiceSelect = (e) => {
    const selectedInvId = e.target.value;
    setFormData(prev => ({ ...prev, invoiceId: selectedInvId }));
    
    if (selectedInvId) {
      const matchedInv = invoices.find(inv => inv._id === selectedInvId);
      if (matchedInv) {
        setFormData(prev => ({
          ...prev,
          partyDetails: {
            name: matchedInv.customerDetails?.name || '',
            gstin: matchedInv.customerDetails?.gstin || '',
            address: matchedInv.customerDetails?.address || '',
            state: matchedInv.customerDetails?.state || '',
            phone: matchedInv.customerDetails?.phone || '',
            email: matchedInv.customerDetails?.email || ''
          }
        }));
        if (matchedInv.items && matchedInv.items.length > 0) {
          setItems(matchedInv.items.map(item => {
            const match = catalogItems.find(ci => ci.name.trim().toLowerCase() === (item.name || '').trim().toLowerCase());
            return {
              catalogItemId: match ? match._id : 'custom',
              name: item.name || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`${formData.noteType === 'CREDIT' ? 'Credit' : 'Debit'} Note created successfully!`);
  ...
  return (
    <>
      ...
      <div className="qsr-page-title-container">
        <h1 className="qsr-page-title">Create Credit / Debit Note</h1>
        <BreadcrumbList items={breadcrumbs} />
      </div>

      <Form onSubmit={handleSubmit}>
        {/* Section 1: Note Configuration */}
        <Card className="note-section-card">
          <Card.Body className="p-4">
            <div className="note-section-title">
              <CsLineIcons icon="settings" size="15" /> Note Configuration
            </div>
            <Row className="g-3">
              <Col xs={12} md={4}>
                <div className="note-field">
                  <Form.Label>Note Type</Form.Label>
                  <Form.Select name="noteType" value={formData.noteType} onChange={handleFormChange}>
                    <option value="CREDIT">Credit Note (Sales Return / Refund)</option>
                    <option value="DEBIT">Debit Note (Purchase Return / Supplier Return)</option>
                  </Form.Select>
                </div>
              </Col>
              <Col xs={12} md={4}>
                <div className="note-field">
                  <Form.Label>Original Invoice Reference (Optional)</Form.Label>
                  <Form.Select name="invoiceId" value={formData.invoiceId} onChange={handleInvoiceSelect}>
                    <option value="">Select Invoice to Auto-fill</option>
                    {invoices.map(inv => (
                      <option key={inv._id} value={inv._id}>{inv.invoiceNumber} - {inv.customerDetails?.name}</option>
                    ))}
                  </Form.Select>
                </div>
              </Col>
              <Col xs={12} md={4}>
                <div className="note-field">
                  ...
                  <Form.Control type="text" name="reason" value={formData.reason} onChange={handleFormChange} required />
                </div>
              </Col>
            </Row>
          </Card.Body>
        ...
        <Card className="note-section-card">
          <Card.Body className="p-4">
            <div className="note-section-title">
              <CsLineIcons icon="user" size="15" /> Party Details (Customer / Vendor)
            </div>
            <Row className="g-3">
              <Col xs={12} md={6}>
                <div className="note-field">
                  <Form.Label>Party Name *</Form.Label>
                  <Form.Control type="text" name="name" value={formData.partyDetails.name} onChange={handlePartyChange} required placeholder="Full name or company" />
                </div>
              </Col>
              <Col xs={12} md={6}>
                <div className="note-field">
                  <Form.Label>GSTIN</Form.Label>
                  <Form.Control type="text" name="gstin" value={formData.partyDetails.gstin} onChange={handlePartyChange} placeholder="15-digit GST number" />
                </div>
              </Col>
              <Col xs={6} md={6}>
                <div className="note-field">
                  <Form.Label>State *</Form.Label>
                  <Form.Control type="text" name="state" value={formData.partyDetails.state} onChange={handlePartyChange} required />
                </div>
              </Col>
              <Col xs={6} md={6}>
                <div className="note-field">
                  <Form.Label>Address</Form.Label>
                  <Form.Control type="text" name="address" value={formData.partyDetails.address} onChange={handlePartyChange} placeholder="Party address" />
                </div>
              </Col>
            </Row>
          </Card.Body>
        ...
        {/* Section 3: Adjusted Line Items */}
        <Card className="note-section-card">
          <Card.Body className="p-4">
            <div className="note-section-title">
              <CsLineIcons icon="box" size="15" /> Adjusted Line Items
            </div>

            {items.map((item, index) => (
              <div className="note-line-card" key={index}>
                <div className="d-flex align-items-center gap-2 mb-3">
                  <div className="note-line-num">{index + 1}</div>
                  <div className="flex-grow-1 note-field mb-0">
                    <CreatableSelect
                      isClearable
                  ...
                  {items.length > 1 && (
                    <button type="button" className="note-remove-btn" onClick={() => removeItem(index)}>×</button>
                  )}
                </div>
                ...
                <div className="d-flex justify-content-between align-items-center mt-3">
                  ...
                  <div className="note-item-total">₹{item.totalAmount.toFixed(2)}</div>
                </div>
              </div>
            ))}
        ...
        <Card className="note-section-card">
          <Card.Body className="p-4">
            <div className="note-section-title">
              <CsLineIcons icon="dollar" size="15" /> Note Summary
            </div>
            <Row className="justify-content-end">
              <Col xs={12} md={6} lg={5}>
                <div className="note-summary-row">
                  <span className="note-summary-label">Subtotal</span>
                  <span className="note-summary-val">₹{summary.subTotal.toFixed(2)}</span>
                </div>
                {summary.totalCGST > 0 && (
                  <div className="note-summary-row">
                    <span className="note-summary-label">Total CGST</span>
                    <span className="note-summary-val">₹{summary.totalCGST.toFixed(2)}</span>
                  </div>
                )}
                {summary.totalSGST > 0 && (
                  <div className="note-summary-row">
                    <span className="note-summary-label">Total SGST</span>
                    <span className="note-summary-val">₹{summary.totalSGST.toFixed(2)}</span>
                  </div>
                )}
                <div className="note-grand-row">
        ...
        <div className="d-flex flex-column flex-sm-row justify-content-end gap-2 mb-5">
          ...
          <Button type="submit" variant="primary" className="rounded-pill px-5" style={{ fontWeight: 700 }}>
            <CsLineIcons icon="save" size="14" className="me-1" /> Save Credit/Debit Note
          </Button>
        </div>
      </Form>
    </>
