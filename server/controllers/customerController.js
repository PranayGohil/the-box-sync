...
exports.addCustomer = async (req, res) => {
    try {
        const { name, email, phone, address, id_proof, date_of_birth, anniversary, tag } = req.body;
        const user_id = req.user;

        // Check if customer with same phone already exists
        const existingCustomer = await Customer.findOne({ phone, user_id });
        if (existingCustomer) {
...
exports.updateCustomer = async (req, res) => {
    try {
        ...
        if (phone && phone !== customer.phone) {
            const existingCustomer = await Customer.findOne({ phone, user_id, _id: { $ne: id } });
            if (existingCustomer) {
                return res.status(400).json({
                    success: false,
