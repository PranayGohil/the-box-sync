...
import { Badge, Button, Col, Row, Modal, Spinner, Alert, Card, Form, Table, Nav, Tab } from 'react-bootstrap';
...
const DailyTrackerTab = ({ brandColor, history }) => {
  ...
  const handleCorrectionRequest = async () => {
    if (!correctionNote.trim()) {
      toast.error('Please enter details of the correction needed.');
      return;
    }
    const targetLog = closingLog || openingLog;
    if (!targetLog) {
      toast.error('No verified log found for today to correct.');
      return;
    }
    try {
      setSendingCorrection(true);
      await createCorrectionRequest({
        log_id: targetLog._id,
    ...
    <>
          ...
          {loading ? (
            ...
            <>
                        ...
                        {openingLog ? (
                          ...
                          <div className="d-flex align-items-center gap-2">
                            ...
                            {selectedDate === format(new Date(), 'yyyy-MM-dd') && (
                              <Button as={Link} to="/operations/daily-opening-stock" size="sm" variant="primary" className="rounded-pill fw-bold py-1">
                                Log Opening
                              </Button>
                            )}
                          </div>
                    ...
                    <Card.Body className="p-3 d-flex align-items-center justify-content-between">
                      ...
                      <div>
                        {closingLog ? (
                          ...
                          <div className="d-flex align-items-center gap-2">
                            <Badge bg="danger" className="px-3 py-2 text-uppercase">Pending</Badge>
                            {selectedDate === format(new Date(), 'yyyy-MM-dd') && openingLog && (
                              <Button as={Link} to="/operations/daily-closing-stock" size="sm" variant="primary" className="rounded-pill fw-bold py-1">
                                Log Closing
                              </Button>
                            )}
                          </div>
              ...
              <div className="d-flex flex-wrap gap-2 mb-4">
                {(openingLog?.log_status === 'manager_verified' || closingLog?.log_status === 'manager_verified') && (
                  <Button variant="outline-warning" className="rounded-pill fw-bold py-2 px-3 small border-2" size="sm" onClick={() => setShowCorrectionModal(true)}>
                    <CsLineIcons icon="edit" size="14" className="me-1" /> Request Stock Correction
                  </Button>
                )}
                {selectedDate === format(new Date(), 'yyyy-MM-dd') && (
                  <>
                    <Button as={Link} to="/operations/add-inventory" variant="outline-primary" className="rounded-pill fw-bold py-2 px-3 small border-2" size="sm">
                      <CsLineIcons icon="plus" size="14" className="me-1" /> Order/Add Inventory
                    </Button>
                  </>
                )}
              </div>
      ...
      <Modal show={showCorrectionModal} onHide={() => !sendingCorrection && setShowCorrectionModal(false)} centered>
        <Modal.Header closeButton={!sendingCorrection}>
          <Modal.Title className="fw-bold">Request Audit Log Correction</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Alert variant="warning" className="py-2">
            <strong>Logged stock is locked.</strong> Provide the details below of which counts are incorrect and what they should be. Admin will update the records.
          </Alert>
          <Form.Group>
            <Form.Label className="fw-semibold">Correction Details <span className="text-danger">*</span></Form.Label>
            <Form.Control
              as="textarea"
              rows={4}
              value={correctionNote}
              onChange={(e) => setCorrectionNote(e.target.value)}
              placeholder="e.g. Tomato opening was 5 kg (misentered as 8 kg)."
              required
            />
          </Form.Group>
        </Modal.Body>
      ...
      <Modal show={showUseModal} onHide={() => !isSubmitting && setShowUseModal(false)} centered>
        <Form onSubmit={handleUseSubmit}>
          <Modal.Header closeButton={!isSubmitting}>
            <Modal.Title className="fw-bold">Mark Ingredient Usage</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <div className="bg-light p-3 rounded-3 mb-3 d-flex justify-content-between align-items-center">
              <div className="fw-bold h5 mb-0">{selectedItem?._id}</div>
              <div className="text-muted fw-bold">Available: {selectedItem?.totalStock} {selectedItem?.unit}</div>
            </div>
            <Form.Group className="mb-3">
              <Form.Label className="fw-semibold">Quantity Used <span className="text-danger">*</span></Form.Label>
              <Form.Control
                type="number"
                step="0.01"
                min="0.01"
            ...
            <Form.Group>
              <Form.Label className="fw-semibold">Comment / Note</Form.Label>
              <Form.Control
                as="textarea"
                rows={2}
                value={useComment}
                onChange={(e) => setUseComment(e.target.value)}
                placeholder="e.g. Deducted for afternoon meal preparation"
              />
            </Form.Group>
          </Modal.Body>
          <Modal.Footer>
            <Button variant="outline-secondary" onClick={() => setShowUseModal(false)} disabled={isSubmitting}>Cancel</Button>
...
const WastageLogsTab = () => {
  ...
  useEffect(() => {
    fetchLogs();
    fetchStockItems();
  }, [fetchLogs, fetchStockItems]);

  const handleItemSelect = (name) => {
    const found = stockItems.find((s) => s._id === name);
    setForm((prev) => ({ ...prev, item_name: name, unit: found?.unit || '' }));
  };
  ...
  const handleLogSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        toast.success('Wastage logged and stock adjusted.');
      ...
      <Col lg={4}>
        <Card className="workstation-card border-0 mb-4">
          <Card.Body className="p-4">
            <h5 className="fw-bold mb-3">Record Waste Entry</h5>
            <Form onSubmit={handleLogSubmit}>
              <Form.Group className="mb-3">
                <Form.Label className="fw-semibold">Select Item <span className="text-danger">*</span></Form.Label>
              ...
              <Row className="g-2 mb-3">
                ...
                <Col xs={4}>
                  <Form.Group>
                    ...
                    <Form.Control className="modern-input bg-light text-center fw-bold" value={form.unit || '—'} readOnly />
                  </Form.Group>
                </Col>
              </Row>

              <Form.Group className="mb-3">
                <Form.Label className="fw-semibold">Wastage Type <span className="text-danger">*</span></Form.Label>
                <div className="d-flex flex-wrap gap-2">
                  {WASTAGE_TYPES.map((t) => (
                    <div key={t.value}>
                      ...
                      />
                      <label
                        className={`btn rounded-pill px-3 py-1 fw-bold small ${form.wastage_type === t.value ? 'btn-primary text-white' : 'btn-outline-light text-dark border-light-subtle'}`}
                        htmlFor={`w-${t.value}`}
                      >
                        {t.label}
                      </label>
              ...
              <Form.Group className="mb-4">
                <Form.Label className="fw-semibold">Reason / Comment</Form.Label>
                <Form.Control
                  as="textarea"
                  rows={2}
                  className="modern-input"
                  ...
                  placeholder="e.g. Expired safety date"
                />
              </Form.Group>

              <Button variant="danger" type="submit" className="w-100 rounded-pill fw-bold py-3 border-0 shadow-sm" disabled={submitting}>
          ...
          <Card.Body className="p-0">
            <div className="p-4 border-bottom bg-white d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
              <div>
                ...
                <p className="text-muted small mb-0">List of logged wastage events</p>
              </div>
              <div className="d-flex align-items-center gap-2">
                  ...
                  <tbody>
                    {logs.map((log) => (
                      <tr key={log._id}>
                        ...
                        <td className="text-muted pe-4" style={{ maxWidth: '200px' }}>{log.reason || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </Table>
...
const AICopilotTab = () => {
  const [messages, setMessages] = useState([
    {
      sender: 'assistant',
      text: "Hello! I am your AI Inventory Co-pilot. I can analyze your daily counts, detect stockout risks, track waste anomalies, or draft supplier orders. How can I help you today?",
    },
  ]);
...
const RequestedInventory = ({ refreshKey, onDeleteClick }) => {
                  ...
                  <tr key={item._id}>
                    ...
                    <td className="text-end">
                      <div className="d-flex justify-content-end gap-1">
                        <Button variant="outline-primary" size="sm" as={Link} to={`/operations/inventory-details/${item._id}`}>
                          View
                        </Button>
...
const CompletedInventory = ({ refreshKey, history }) => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
...
const RejectedInventory = ({ refreshKey, history }) => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
...
const InventoryHistory = () => {
  ...
  return (
    <div className="workstation-container">
      ...
      <HtmlHead title={title} description="Unified Daily Stock and Inventory tracker workstation." />
      <div className="container-fluid px-lg-5">
        ...
        {activeTab === 'tracker' && <DailyTrackerTab brandColor={brandColor} history={history} />}
        {activeTab === 'stock' && <StockControlTab brandColor={brandColor} />}
        {activeTab === 'requests' && (
          <>
            <RequestedInventory refreshKey={refreshKey} onDeleteClick={(item) => { setSelectedItem(item); setShowDeleteModal(true); }} />
            <CompletedInventory refreshKey={refreshKey} history={history} />
            <RejectedInventory refreshKey={refreshKey} history={history} />
          </>
        )}
          ...
          <Modal.Body className="py-4">
            <div className="d-flex align-items-center mb-3">
              ...
              <div>
                ...
                <p className="mb-1 text-muted small">This clears the inventory request log from your history.</p>
