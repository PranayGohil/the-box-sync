...
import { Row, Col, Card, Button, Alert, Spinner, Badge, Modal, Form, ProgressBar, Toast, ToastContainer, Table } from 'react-bootstrap';
...
export default function ViewStaffPayroll() {
  ...
  const fetchPayroll = async () => {
    ...
    setError(null);
    try {
      const res = await axios.get(`${process.env.REACT_APP_API}/payroll/get/${staffId}`, authHeader());
    ...
    try {
      ...
      filteredPayroll.forEach((p) => {
        recordsData.push([
          MONTH_NAMES[p.month],
          p.year,
          p.working_days_in_month,
          p.present_days,
          p.absent_days,
          staffData.salary || p.base_salary || 0,
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(10);
      doc.text(
        `Staff ID: ${staffData.staff_id}  |  Position: ${staffData.position}  |  Base Salary: ₹${staffData.salary}  |  OT Rate: ₹${staffData.overtime_rate}/hr`,
        20,
        y
      );
      y += 6;
        ...
        sections: [
          {
            children: [
              ...
              new Paragraph({ text: '' }),
              new Paragraph({
                text: `Total Months: ${careerStats.total_months} | Total Earned: ₹${careerStats.total_earned} | Total Paid: ₹${careerStats.total_paid} | Unpaid: ₹${careerStats.total_unpaid}`,
              }),
              new Paragraph({ text: '' }),
              new DocxTable({ rows }),
            ],
    ...
    <div className="container-fluid qsr-page-container">
      ...
      <div className="qsr-page-title-container">
        <Row className="align-items-center">
          <Col>
            <h1 className="qsr-page-title">{staffData ? `${staffData.f_name} ${staffData.l_name}'s Payroll` : main_title}</h1>
      ...
      <Row className="g-3 mb-4">
        {[
          ...
          {
            label: 'Total Earned',
            value: `₹${(careerStats.total_earned || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
            icon: 'dollar',
            color: 'text-dark',
          },
          {
            label: 'Total Paid',
            value: `₹${(careerStats.total_paid || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
            icon: 'check-circle',
            color: 'text-success',
          },
          {
            label: 'Total Unpaid',
            value: `₹${(careerStats.total_unpaid || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
            icon: 'clock',
            color: 'text-warning',
          },
        ].map((s) => (
                ...
                <Table hover className="mb-0">
                    ...
                    {filteredPayroll.map((p) => (
                      <tr key={p._id}>
                        <td>
                          ...
                          {p.paid_date && <small className="text-muted">Paid: {p.paid_date}</small>}
                        </td>
                        ...
                        <td className="text-end fw-bold">₹{p.net_salary.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                        <td className="text-center">
                          {p.status === 'paid' ? (
                            <Badge bg="success">
                  ...
                  <tfoot className="table-light fw-bold">
                    <tr>
                      ...
                      <td colSpan={7} />
                      <td className="text-end text-danger">₹{filteredStats.total_deductions.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                      <td className="text-end">₹{filteredStats.total_net.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                      <td colSpan={2} />
                    </tr>
                  </tfoot>
                </Table>
      ...
      <Modal show={showExportModal} onHide={() => setShowExportModal(false)}>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowExportModal(false)} className="btn-icon">
            Cancel
          </Button>
      ...
      <Modal show={showDetailModal} onHide={() => setShowDetailModal(false)} centered size="lg">
        <Modal.Header closeButton>
          <Modal.Title>Payroll Details — {selectedPayroll ? `${MONTH_NAMES[selectedPayroll.month]} ${selectedPayroll.year}` : ''}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedPayroll && (
            <Row className="g-3">
                ...
                <Row className="g-3">
                  <Col xs={6} md={3}>
                    ...
                    <div className="fw-bold">
                      {MONTH_NAMES[selectedPayroll.month]} {selectedPayroll.year}
                    </div>
                  </Col>
                  <Col xs={6} md={3}>
              ...
              <Col md={6}>
                <div className="border rounded p-3 bg-light h-100">
                  ...
                  {selectedPayroll.earned_breakdown && (
                    <>
                      {selectedPayroll.earned_breakdown.basic > 0 && (
                        <div className="d-flex justify-content-between mb-1">
                          <span>Basic</span>
                          <span>₹{selectedPayroll.earned_breakdown.basic.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                      )}
                      {selectedPayroll.earned_breakdown.hra > 0 && (
                        <div className="d-flex justify-content-between mb-1">
                          <span>HRA</span>
                          <span>₹{selectedPayroll.earned_breakdown.hra.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                      )}
                      {selectedPayroll.earned_breakdown.conveyance > 0 && (
                        <div className="d-flex justify-content-between mb-1">
                          <span>Conveyance</span>
                          <span>₹{selectedPayroll.earned_breakdown.conveyance.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                      )}
                      {selectedPayroll.earned_breakdown.medical > 0 && (
                        <div className="d-flex justify-content-between mb-1">
                          <span>Medical</span>
                          <span>₹{selectedPayroll.earned_breakdown.medical.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                      )}
                      {selectedPayroll.earned_breakdown.special > 0 && (
                        <div className="d-flex justify-content-between mb-1">
                          <span>Special</span>
                          <span>₹{selectedPayroll.earned_breakdown.special.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                      )}
                      {selectedPayroll.earned_breakdown.other > 0 && (
                        <div className="d-flex justify-content-between mb-1">
                          <span>Other</span>
                          <span>₹{selectedPayroll.earned_breakdown.other.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                      )}
                    </>
                  )}
                  {!selectedPayroll.earned_breakdown && (
                    <div className="d-flex justify-content-between mb-1">
                      ...
                      <span>₹{(selectedPayroll.earned_salary || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                    </div>
                  )}
                  ...
                  {selectedPayroll.overtime_pay > 0 && (
                    <div className="d-flex justify-content-between mb-1 mt-2">
                      <span>
                        Overtime Pay ({selectedPayroll.overtime_hours}h @ ₹{selectedPayroll.overtime_rate}/h)
                      </span>
                      <span className="text-primary">₹{(selectedPayroll.overtime_pay || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                    </div>
                  )}
                  {selectedPayroll.bonus > 0 && (
                    <div className="d-flex justify-content-between mb-1">
                      <span>Bonus</span>
                      <span className="text-success">₹{(selectedPayroll.bonus || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                    </div>
                  )}
              ...
              <Col md={6}>
                <div className="border rounded p-3 bg-light h-100">
                  ...
                  {selectedPayroll.deduction_breakdown && (
                    <>
                      {selectedPayroll.deduction_breakdown.pf > 0 && (
                        <div className="d-flex justify-content-between mb-1">
                          <span>Provident Fund (PF)</span>
                          <span>₹{selectedPayroll.deduction_breakdown.pf.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                      )}
                      {selectedPayroll.deduction_breakdown.esi > 0 && (
                        <div className="d-flex justify-content-between mb-1">
                          <span>ESI</span>
                          <span>₹{selectedPayroll.deduction_breakdown.esi.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                      )}
                      {selectedPayroll.deduction_breakdown.pt > 0 && (
                        <div className="d-flex justify-content-between mb-1">
                          <span>Professional Tax (PT)</span>
                          <span>₹{selectedPayroll.deduction_breakdown.pt.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                      )}
                  ...
                  <div className="d-flex justify-content-between fw-bold text-danger">
                    ...
                    <span>
                      ₹
                      {((selectedPayroll.deduction_breakdown?.total_statutory || 0) + (selectedPayroll.deductions || 0)).toLocaleString('en-IN', {
              ...
              <Col xs={12}>
                <div
                  className="mt-2 p-3 rounded d-flex justify-content-between align-items-center"
                  style={{ background: '#f0f4ff', border: '1px solid #c7d7ff' }}
                >
                  <span className="fw-bold">Net Salary</span>
                  <h4 className="mb-0 text-primary">₹{selectedPayroll.net_salary.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h4>
                </div>
              </Col>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowDetailModal(false)} className="btn-icon">
            Close
          </Button>
        </Modal.Footer>
      </Modal>
