        ...
        try {
            ...
            autoTable(doc, {
                ...
                body: filteredPayroll.map((p) => [
                    ...
                    p.present_days,
                    p.absent_days,
                    `₹${p.earned_salary.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
                    `₹${p.overtime_pay.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
                    `₹${p.bonus.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
                    `₹${p.deductions.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
        ...
        <>
            ...
            <div className="page-title-container mb-3">
                <Row className="align-items-center">
                    ...
                    <Col xs="auto">
                        <Button variant="outline-secondary" onClick={() => history.push('/staff/payroll')}>
            ...
            <Card className="mb-4">
                <Card.Body>
                    <Row className="g-3 align-items-end">
                        ...
                        <Col md={2}>
                            <Button variant="outline-secondary" className="w-100" onClick={() => { setYearFilter('all'); setStatusFilter('all'); }}>
                                <CsLineIcons icon="close" className="me-1" />Clear
                            </Button>
                        </Col>
                    </Row>
                </Card.Body>
                                    ...
                                    <tbody>
                                        {filteredPayroll.map((p) => (
                                            <tr key={p._id}>
                                                ...
                                                <td className="text-end">₹{p.base_salary.toLocaleString('en-IN')}</td>
                                                <td className="text-end text-success">₹{p.earned_salary.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                                ...
                                                <td className="text-end">
                                                    {p.bonus > 0
                                                        ? <span className="text-success">₹{p.bonus.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                                                        : <span className="text-muted">—</span>}
                                                </td>
                                                <td className="text-end">
                                                    {p.deductions > 0
                                                        ? <span className="text-danger">₹{p.deductions.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                                                        : <span className="text-muted">—</span>}
                                                </td>
            ...
            <Modal show={showDetailModal} onHide={() => setShowDetailModal(false)} centered size="lg">
                ...
                <Modal.Body>
                    {selectedPayroll && (
                        <Row className="g-3">
                            {[
                                { label: 'Month / Year', value: `${MONTH_NAMES[selectedPayroll.month]} ${selectedPayroll.year}` },
                                { label: 'Working Days', value: selectedPayroll.working_days_in_month },
                                { label: 'Present Days', value: selectedPayroll.present_days, color: 'text-success' },
                                { label: 'Absent Days', value: selectedPayroll.absent_days, color: selectedPayroll.absent_days > 0 ? 'text-danger' : '' },
                                { label: 'Base Salary', value: `₹${selectedPayroll.base_salary.toLocaleString('en-IN')}` },
                                { label: 'Earned Salary', value: `₹${selectedPayroll.earned_salary.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, color: 'text-success' },
                                { label: 'Overtime Hours', value: `${selectedPayroll.overtime_hours} hrs` },
                                { label: 'OT Rate', value: `₹${selectedPayroll.overtime_rate}/hr` },
                                { label: 'Overtime Pay', value: `₹${selectedPayroll.overtime_pay.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, color: 'text-primary' },
                                { label: 'Bonus', value: `₹${selectedPayroll.bonus.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, color: 'text-success' },
                                { label: 'Deductions', value: `₹${selectedPayroll.deductions.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, color: selectedPayroll.deductions > 0 ? 'text-danger' : '' },
                                { label: 'Deduction Reason', value: selectedPayroll.deduction_reason || '—' },
                            ].map((item) => (
