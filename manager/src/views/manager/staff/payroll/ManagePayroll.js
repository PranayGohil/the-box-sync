  ...
  return (
    <>
      ...
      <div className="container-fluid qsr-page-container">
            ...
            {[
              {
                ...
                value: `₹${(totals.total_net_salary || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
                icon: 'dollar',
                color: 'text-primary',
              },
              {
                label: 'Paid',
                value: `₹${(totals.total_paid || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
                icon: 'check-circle',
                color: 'text-success',
              },
              {
                label: 'Unpaid',
                value: `₹${(totals.total_unpaid || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
                icon: 'clock',
                color: 'text-warning',
              },
        ...
        {selectedIds.size > 0 && (
          <div className="d-flex align-items-center gap-2 mb-3 p-3 bg-light rounded">
            ...
            <Button variant="success" size="sm" className="btn-icon" onClick={() => handleMarkPaid(selectedIds)} disabled={isMarkingPaid}>
              {isMarkingPaid ? (
            ...
            <Button variant="warning" size="sm" className="btn-icon" onClick={() => handleMarkUnpaid(selectedIds)} disabled={isMarkingPaid}>
              <CsLineIcons icon="rotate-left" className="me-1" />
              Mark Unpaid
            </Button>
            <Button variant="outline-secondary" size="sm" className="btn-icon" onClick={() => setSelectedIds(new Set())}>
              Clear
            </Button>
          </div>
        )}
              ...
              <div className="table-responsive">
                <Table hover className="mb-0">
                  <thead className="table-light">
                    <tr>
                      <th style={{ width: '40px' }}>
                        <Form.Check type="checkbox" checked={allSelected} onChange={toggleSelectAll} disabled={payrollRows.length === 0} />
                      </th>
                      <th>Staff</th>
                  ...
                  <tbody>
                    {summary.map(({ staff, payroll }) => (
                      <tr key={staff._id} className={selectedIds.has(payroll?._id) ? 'table-primary' : ''}>
                        ...
                        <td>
                          {payroll ? (
                            <Form.Check type="checkbox" checked={selectedIds.has(payroll._id)} onChange={() => toggleSelect(payroll._id)} />
                          ) : (
                        ...
                        {payroll ? (
                          <>
                            <td className="text-center">
                              <Badge bg="success">{payroll.present_days}</Badge>
                            </td>
                            <td className="text-center">
                            ...
                            <td className="text-end text-success">
                              ₹{(payroll.earned_breakdown?.total_gross || payroll.earned_salary || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                            </td>
              ...
              <div className="d-flex flex-wrap gap-3 align-items-center">
                ...
                <div className="d-flex align-items-center">
                  <Badge bg="danger" className="me-2 px-2 py-1">
                    <CsLineIcons icon="info-hexagon" size={12} />
                  </Badge>
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)} centered size="lg">
            ...
            <>
              ...
              <Row className="g-3">
                ...
                <Col xs={12} md={6}>
                  ...
                  <Form.Control type="number" min={0} value={editForm.bonus} onChange={(e) => setEditForm({ ...editForm, bonus: e.target.value })} />
                </Col>
                <Col xs={12} md={6}>
                  <Form.Label>Deductions (₹)</Form.Label>
                  <Form.Control type="number" min={0} value={editForm.deductions} onChange={(e) => setEditForm({ ...editForm, deductions: e.target.value })} />
                </Col>
                <Col xs={12}>
                ...
                <Col xs={12}>
                  ...
                  <Form.Control as="textarea" rows={2} value={editForm.notes} onChange={(e) => setEditForm({ ...editForm, notes: e.target.value })} />
                </Col>
              </Row>
              ...
              <div className="mt-4 p-3 rounded" style={{ background: '#f0f4ff', border: '1px solid #c7d7ff' }}>
                <div className="d-flex justify-content-between align-items-center">
                  ...
                  <h4 className="mb-0 text-primary">₹{editNetPreview().toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h4>
                </div>
              </div>
            </>
          )}
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowEditModal(false)} className="btn-icon">
            Cancel
          </Button>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowDeleteModal(false)} className="btn-icon">
            Cancel
          </Button>
