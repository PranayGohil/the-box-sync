  ...
  return (
    <>
      ...
      <div className="container-fluid qsr-page-container">
            ...
            <Row className="g-3 align-items-end">
              ...
              <Col xs={12} md={2}>
                ...
                <Form.Control
                  ...
                  value={workingDays}
                  onChange={(e) => {
                    setWorkingDays(Number(e.target.value) || 26);
                    setPreviewFetched(false);
                  }}
                />
              </Col>
        ...
        {/* ── Generate Result ── */}
        {generateResult && (
          <Alert variant="success" className="mb-4" dismissible onClose={() => setGenerateResult(null)}>
            <div className="d-flex align-items-center justify-content-between flex-wrap gap-2">
              <div>
                ...
                {generateResult.data?.errors?.length > 0 && (
                  <div className="mt-1 text-danger small">{generateResult.data.errors.length} error(s) — check console for details.</div>
                )}
              </div>
              <Button variant="success" size="sm" className="btn-icon" onClick={() => history.push(`/staff/payroll/${month}/${year}`)}>
                <CsLineIcons icon="eye" className="me-2" />
                View Payroll
              </Button>
            </div>
          ...
          <Card className="mb-4">
            <Card.Header className="d-flex justify-content-between align-items-center flex-wrap gap-2">
              <div>
                ...
                <small className="text-muted">Review and adjust overtime, bonus, deductions before generating</small>
              </div>
                ...
                <Table className="mb-0" hover>
                    ...
                    {previews.map((p) => {
                      ...
                      return (
                        <tr key={p.staff_id}>
                          ...
                          {/* OT Rate */}
                          <td className="text-end text-muted">₹{p.overtime_rate.toLocaleString('en-IN')}</td>

                          {/* OT Pay */}
                          <td className="text-end text-primary">₹{ot_pay.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                          ...
                          <td>
                            <div className="text-danger small mb-1">Stat: ₹{(p.deduction_breakdown?.total_statutory || 0).toLocaleString('en-IN')}</div>
                            <Form.Control
                              type="number"
                              size="sm"
                              min={0}
                          ...
                          <td className="text-end fw-bold fs-6">
                            <span className={net < 0 ? 'text-danger' : 'text-dark'}>₹{net.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                          </td>

                          {/* Already exists badge */}
                          <td className="text-center">
                            {p.already_exists ? (
                              <Badge bg={p.existing_status === 'paid' ? 'success' : 'warning'}>{p.existing_status === 'paid' ? 'Paid' : 'Will Update'}</Badge>
                            ) : (
                  ...
                  <tfoot className="table-light fw-bold">
                    <tr>
                      ...
                      <td colSpan={7} />
                      <td className="text-end fs-6">₹{previewTotals.total_net.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                      <td />
                    </tr>
                  </tfoot>
                </Table>
            ...
            <Card.Footer className="d-flex justify-content-between align-items-center flex-wrap gap-2">
              ...
              <Button variant="success" size="lg" className="btn-icon" onClick={() => setShowConfirmModal(true)} disabled={isGenerating}>
                {isGenerating ? (
                  <>
                    <Spinner as="span" animation="border" size="sm" className="me-2" />
                    Generating...
      ...
      <Modal show={showConfirmModal} onHide={() => setShowConfirmModal(false)} centered>
        ...
        <Modal.Body>
          <p>
            ...
            for{' '}
            <strong>
              {previews.length} staff member{previews.length !== 1 ? 's' : ''}
            </strong>
            .
          </p>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowConfirmModal(false)} className="btn-icon">
            Cancel
          </Button>
