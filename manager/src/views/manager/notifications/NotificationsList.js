...
const NotificationsList = () => {
  ...
  const description = 'View and manage restaurant notifications and alerts';
  ...
  const getNotificationLink = (item) => {
    if (item.type === 'inventory_approved' || item.type === 'inventory_rejected') {
      return '/operations/inventory-history';
    }
  ...
  const getNotificationText = (item) => {
    if (item.type === 'inventory_approved') {
      return `Inventory request approved/delivered (${item.data?.category || 'items'})!`;
    }
    if (item.type === 'inventory_rejected') {
      return `Inventory request rejected: ${item.data?.reject_reason || 'N/A'}`;
    }
    if (item.type === 'web_order_recieved') {
          ...
          <Card.Body className="p-4 p-lg-5">
            ...
            <div className="inventory-history-section-title-wrapper mb-4">
              ...
              <div>
                ...
                <p className="text-muted small mb-0">Manage manager notifications and updates</p>
              </div>
            </div>
            ...
            {status === 'loading' && items.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ flex: 0.8 }}>
                        ...
                        <div
                          className={`sw-5 sh-5 d-flex align-items-center justify-content-center rounded-circle flex-shrink-0 ${!item.read ? 'bg-primary text-white' : 'bg-separator-light text-primary'
                            }`}
              ...
              <Alert variant="light" className="text-center py-5 border-dashed rounded-4">
                ...
                <p className="text-muted mb-0 small">You have no active notifications.</p>
              </Alert>
            )}
          </Card.Body>
        </Card>
