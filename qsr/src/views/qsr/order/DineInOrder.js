...
const DineInOrder = () => {
  ...
  const title = `${isEditMode ? 'Edit' : 'New'} Dine-In Order`;
  ...
  async function fetchOrderDetails() {
    try {
      ...
      const detectedType = order.order_type || 'Dine In';
      ...
      if (detectedType === 'Dine In') {
        ...
        custInfo = { name: order.customer_name || '', phone: order.customer_phone || '', address: '', total_persons: '', waiter: '', table_no: '', comment: order.comment || '' };
      }
      ...
      initialStateRef.current = { orderItems: JSON.parse(JSON.stringify(items)), customerInfo: JSON.parse(JSON.stringify(custInfo)), paid_amount: order.paid_amount || 0 };
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem('token');
    if (orderId) { fetchOrderDetails(); } else { setIsInitialized(true); }
  ...
  const computeKOTDelta = (currentItems, snapshotItems) => {
    ...
    for (const item of currentItems) {
      if (!['Completed', 'Cancelled', 'Container Charge'].includes(item.status)) {
        ...
        if (!prev) { delta.push({ ...item }); } else if (item.quantity > prev.quantity) { delta.push({ ...item, quantity: item.quantity - prev.quantity }); }
      }
    }
    return delta;
  };
  ...
  const buildPayload = (status, completeAll = false) => {
    const orderData = {
      ...
      order_items: orderItems.map((item) => ({
        ...
        status: completeAll ? 'Completed' : (status === 'KOT' || status === 'Paid') ? (item.status === 'Pending' ? 'Preparing' : item.status) : (status === 'Save' ? (item.status || 'Pending') : item.status),
      })),
      order_status: status,
  ...
  const handleKotAndPrint = async () => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        setIsDirty(false); setOrderStatus('KOT');
        ...
        printKOTSlip({ orderNo, orderType, tokenNumber, tableNo: customerInfo.table_no || tableInfo.table_no, items: delta, kotNo, timestamp }, userData, setKotPrinting);
        toast.success(`KOT #${kotNo} sent to kitchen!`);
        if (!orderId && savedId) {
          allowNavigationRef.current = true;
          if (orderType === 'Dine In' && tableId) { window.location.href = `/order/dine-in?tableId=${tableId}&orderId=${savedId}&mode=edit`; } else { window.location.href = `/order/new?orderId=${savedId}&mode=edit`; }
        } else { fetchOrderDetails(); }
      }
    } catch (err) { console.error('Error saving KOT:', err); alert('Error saving KOT. Please try again.'); }
    finally { setIsLoading(false); }
  };

  const handleReprintKOT = (record) => { printKOTSlip({ orderNo, orderType, tokenNumber, tableNo: customerInfo.table_no || tableInfo.table_no, items: record.items, kotNo: record.kotNo, timestamp: record.timestamp }, userData, setKotPrinting); };
  ...
  const handleSaveOrder = async (status = 'Save') => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        const savedId = response.data.orderId || response.data.order?._id || orderId;
        allowNavigationRef.current = true;
        initialStateRef.current = { orderItems: JSON.parse(JSON.stringify(orderItems)), customerInfo: JSON.parse(JSON.stringify(customerInfo)), paid_amount: (response.data.order?.paid_amount || initialStateRef.current.paid_amount || 0) };
        ...
        if (status !== 'Paid' && status !== 'KOT') { history.push('/dashboard'); } else { fetchOrderDetails(); toast.success('Order saved and marked as Paid!'); history.push('/dashboard'); }
      }
  ...
  const handleCancelOrder = async () => {
    ...
    setIsLoading(true);
    try {
      const payload = { orderInfo: { order_id: orderId, order_status: 'Cancelled', order_items: orderItems.map((item) => ({ ...item, status: 'Cancelled' })) }, tableId: orderType === 'Dine In' ? tableId : undefined };
      const token = localStorage.getItem('token');
      const response = await API_MAP[orderType](payload, token);
  ...
  const handlePayment = async () => { if (paymentData.paidAmount <= 0) { alert('Please enter a valid paid amount'); return; } await handleSaveOrder('Paid'); };
    ...
    <>
      ...
      <div className="pos-wrapper">
        <div className="pos-topbar">
          <Button className="custom-btn-outline" style={{ padding: '0.35rem 1rem', flexShrink: 0 }} onClick={() => handleNavigation('/dashboard')}><CsLineIcons icon="arrow-left" size="13" className="me-1" />Back</Button>
          <div className="pos-title flex-grow-1">{title}{orderType === 'Dine In' && (tableInfo.table_no || customerInfo.table_no) && <span className="ms-2 fw-normal text-muted" style={{ fontSize: '14px' }}>— Table {tableInfo.table_no || customerInfo.table_no}</span>}</div>
          {tokenNumber && <div style={{ border: '1.5px solid #23b3f4', borderRadius: '50px', padding: '3px 12px', color: '#23b3f4', fontWeight: 700, fontSize: '12px', flexShrink: 0 }}>Token #{tokenNumber}</div>}
          {orderStatus && <div style={{ border: '1.5px solid #6c757d', borderRadius: '50px', padding: '3px 12px', color: '#6c757d', fontWeight: 700, fontSize: '12px', flexShrink: 0 }}>{orderStatus}</div>}
          <Form.Select size="sm" value={orderType} onChange={(e) => handleOrderTypeChange(e.target.value)} disabled={isEditMode || !!tableId} style={{ maxWidth: '130px', borderRadius: '50px', borderColor: 'rgba(35,179,244,0.35)', color: '#23b3f4', fontWeight: 700, fontSize: '13px', flexShrink: 0 }}>{ORDER_TYPES.map((t) => (<option key={t} value={t}>{t}</option>))}</Form.Select>
          <div className="text-muted small fw-semibold" style={{ flexShrink: 0 }}>{new Date().toLocaleDateString('en-IN')}</div>
        </div>
        <div className="pos-body">
          <MenuGrid filteredMenuData={filteredMenuData} categories={categories} selectedCategory={selectedCategory} setSelectedCategory={setSelectedCategory} searchText={searchText} setSearchText={setSearchText} showSpecial={showSpecial} setShowSpecial={setShowSpecial} showCategories={showCategories} setShowCategories={setShowCategories} addItemToOrder={addItemToOrder} orderItems={orderItems} showParcelCharge={showParcelUI ? showParcelCharge : false} setShowParcelCharge={showParcelUI ? setShowParcelCharge : undefined} containerCharges={showParcelUI ? containerCharges : []} addParcelCharge={showParcelUI ? addParcelCharge : undefined} />
          <div className="pos-order-panel">
            <div className="pos-order-header"><div className="d-flex justify-content-between align-items-center"><div className="fw-bold" style={{ color: '#23b3f4', fontSize: '13px' }}>{orderType === 'Dine In' && (tableInfo.table_no || customerInfo.table_no) ? `T-${tableInfo.table_no || customerInfo.table_no}` : 'Order'} ({orderItems.length})</div>{tokenNumber && <div style={{ background: 'rgba(35,179,244,0.1)', borderRadius: '50px', padding: '2px 10px', color: '#23b3f4', fontWeight: 800, fontSize: '11px' }}>#{tokenNumber}</div>}</div></div>
            <div className="pos-customer-section"><CustomerInfoForm customerInfo={customerInfo} setCustomerInfo={setCustomerInfo} tableInfo={tableInfo} waiterOptions={waiterOptions} orderStatus={orderStatus} visibleFields={visibleFields} requiredFields={requiredFields} /><div className="mt-2"><Form.Control as="textarea" rows={1} value={customerInfo.comment} onChange={(e) => setCustomerInfo((prev) => ({ ...prev, comment: e.target.value }))} placeholder="Notes..." style={{ borderRadius: '6px', border: '1.5px solid rgba(226,232,240,0.9)', fontSize: '11.5px', height: '28px', resize: 'none', color: '#333', background: '#f8fafc' }} /></div></div>
            <div className="pos-cart-section"><OrderCartTable orderItems={orderItems} updateItemQuantity={updateItemQuantity} removeItem={removeItem} /></div>
            <div className="pos-total-section"><PaymentSummaryBox orderItems={orderItems} isDirty={isDirty} orderStatus={orderStatus} isLoading={isLoading} printing={printing} paymentData={paymentData} orderId={orderId} handleSaveOrder={handleSaveOrder} handleOpenPaymentModal={handleOpenPaymentModal} setShowCancelModal={setShowCancelModal} handlePrint={handlePrint} history={history} setShowCartSheet={setShowCartSheet} onKotAndPrint={handleKotAndPrint} kotPrinting={kotPrinting} kotHistory={kotHistory} onReprintKOT={handleReprintKOT} paymentHistory={paymentHistory} alreadyPaid={parseFloat(initialStateRef.current?.paid_amount) || 0} canKOT={canKOT} /></div>
          </div>
        </div>
      </div>
      <PaymentModal showPaymentModal={showPaymentModal} setShowPaymentModal={setShowPaymentModal} paymentData={paymentData} setPaymentData={setPaymentData} isLoading={isLoading} handleDiscountTypeChange={handleDiscountTypeChange} handleDiscountValueChange={handleDiscountValueChange} handlePaidAmountChange={handlePaidAmountChange} handlePayment={handlePayment} orderItems={orderItems} customerInfo={customerInfo} orderType={orderType} orderId={orderId} orderNo={orderNo} alreadyPaid={parseFloat(initialStateRef.current.paid_amount) || 0} />
      <LeaveConfirmationModal showLeaveModal={showLeaveModal} setShowLeaveModal={setShowLeaveModal} setNextLocation={setNextLocation} orderStatus={orderStatus} allowNavigationRef={allowNavigationRef} setIsDirty={setIsDirty} nextLocation={nextLocation} history={history} handleSaveOrder={handleSaveOrder} isLoading={isLoading} />
      ...
      <BottomCartSheet showCartSheet={showCartSheet} setShowCartSheet={setShowCartSheet} orderItems={orderItems} updateItemQuantity={updateItemQuantity} removeItem={removeItem} isDirty={isDirty} orderStatus={orderStatus} isLoading={isLoading} printing={printing} paymentData={paymentData} orderId={orderId} handleSaveOrder={handleSaveOrder} handleOpenPaymentModal={handleOpenPaymentModal} setShowCancelModal={setShowCancelModal} handlePrint={handlePrint} history={history} alreadyPaid={parseFloat(initialStateRef.current?.paid_amount) || 0} canKOT={canKOT} onKotAndPrint={handleKotAndPrint} kotPrinting={kotPrinting} kotHistory={kotHistory} onReprintKOT={handleReprintKOT} paymentHistory={paymentHistory}><div className="d-flex align-items-center justify-content-between mb-3"><h6 className="mb-0 fw-bold text-muted border-bottom pb-2 flex-grow-1">Customer Details</h6><Form.Select size="sm" value={orderType} onChange={(e) => handleOrderTypeChange(e.target.value)} disabled={isEditMode || !!tableId} style={{ maxWidth: '130px', borderRadius: '50px', borderColor: 'rgba(35,179,244,0.3)', color: '#23b3f4', fontWeight: 700 }}>{ORDER_TYPES.map((t) => (<option key={t} value={t}>{t}</option>))}</Form.Select></div><CustomerInfoForm customerInfo={customerInfo} setCustomerInfo={setCustomerInfo} tableInfo={tableInfo} waiterOptions={waiterOptions} orderStatus={orderStatus} visibleFields={visibleFields} requiredFields={requiredFields} /><Form.Group className="mb-3"><Form.Label className="fw-semibold small text-muted">Special Instructions</Form.Label><Form.Control as="textarea" rows={2} value={customerInfo.comment} onChange={(e) => setCustomerInfo((prev) => ({ ...prev, comment: e.target.value }))} placeholder="Any special instructions..." style={{ borderRadius: '12px', border: '1px solid rgba(35,179,244,0.2)', fontSize: '13px' }} /></Form.Group></BottomCartSheet>
      <MobileCartBar
