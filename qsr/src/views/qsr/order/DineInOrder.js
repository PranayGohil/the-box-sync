...
import { getTableById, getOrderById, getUserTaxInfo, createOrUpdateDineInOrder } from 'api/orderService';
...
const DineInOrder = () => {
  ...
  const [customerInfo, setCustomerInfo] = useState({ name: '', total_persons: '', waiter: '', comment: '' });
  ...
  const initialStateRef = useRef({ orderItems: [], customerInfo: { name: '', total_persons: '', waiter: '', comment: '' } });
  ...
  async function fetchOrderDetails() {
    try {
      ...
      const custInfo = { name: order.customer_name || '', total_persons: order.total_persons || '', waiter: order.waiter || '', comment: order.comment || '' };
      setOrderItems(items);
  ...
  useEffect(() => {
    const token = localStorage.getItem('token');
    if (tableId) getTableById(tableId, token).then((r) => setTableInfo(r.data.data)).catch(console.error);
  ...
  const handleSaveOrder = async (status = 'Save') => {
    ...
    try {
      const orderData = {
        table_no: tableInfo.table_no, table_area: tableInfo.area,
        order_type: 'Dine In',
        ...
        order_status: status,
        customer_name: customerInfo.name, total_persons: customerInfo.total_persons,
        comment: customerInfo.comment, waiter: customerInfo.waiter,
      ...
      const payload = { orderInfo: { ...orderData, order_id: orderId }, customerInfo: { name: customerInfo.name }, tableId };
      const token = localStorage.getItem('token');
  ...
  const handleCancelOrder = async () => {
    ...
    setIsLoading(true);
    try {
      const payload = { orderInfo: { order_id: orderId, order_status: 'Cancelled', order_items: orderItems.map((item) => ({ ...item, status: 'Cancelled' })) }, tableId };
      const token = localStorage.getItem('token');
      const response = await createOrUpdateDineInOrder(payload, token);
    ...
    <>
      ...
      <div className="d-flex d-lg-none justify-content-between align-items-center mb-2 px-2 mt-2">
        <div className="d-flex align-items-center gap-2">
          <h5 className="mb-0 fw-bold">Dine-In {tableInfo.table_no ? `— Table ${tableInfo.table_no}` : ''}</h5>
          {orderStatus && <Badge bg="secondary">{orderStatus}</Badge>}
        </div>
        <span className="text-muted small">{new Date().toLocaleDateString('en-IN')}</span>
      </div>
        ...
        <Col lg="4" xs="12" className="pe-lg-0 px-1 px-lg-0">
          <Card className="h-100 mobile-transparent-card">
            ...
            <Card.Body className="p-0 p-lg-3">
              {/* Desktop only: customer form + cart */}
              <div className="d-none d-lg-block">
                <CustomerInfoForm
                  customerInfo={customerInfo} setCustomerInfo={setCustomerInfo}
                  tableInfo={tableInfo} orderStatus={orderStatus}
                  requiredFields={{ name: false, total_persons: false, waiter: false }}
                  visibleFields={{ name: true, total_persons: true, waiter: false }}
                />
      ...
      <PaymentModal
        ...
        orderItems={orderItems} customerInfo={customerInfo} orderType="Dine In" orderId={orderId} orderNo={orderNo}
      />
