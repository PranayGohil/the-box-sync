...
export const LeaveConfirmationModal = ({
  showLeaveModal, setShowLeaveModal, setNextLocation, orderStatus,
  allowNavigationRef, setIsDirty, nextLocation, history, handleSaveOrder, isLoading
}) => {
  return (
    <Modal show={showLeaveModal} onHide={() => { setShowLeaveModal(false); setNextLocation(null); }} centered backdrop="static" keyboard={false}>
      ...
      <Modal.Body>
        ...
        <p>
          Please{' '}
          {orderStatus === 'Save' && <><strong>Save</strong> or</>}{' '}
      ...
      <Modal.Footer>
        <Button variant="danger" onClick={() => {
          ...
          if (nextLocation) setTimeout(() => history.push(nextLocation), 0);
        }}>
          Discard & Leave
        </Button>
        {orderStatus === 'Save' && (
...
export const CancelOrderModal = ({ showCancelModal, setShowCancelModal, handleCancelOrder, isLoading }) => {
  return (
    <Modal show={showCancelModal} onHide={() => setShowCancelModal(false)} centered backdrop="static" keyboard={false}>
      ...
      <Modal.Footer>
        <Button variant="dark" onClick={() => setShowCancelModal(false)} disabled={isLoading}>Keep</Button>
