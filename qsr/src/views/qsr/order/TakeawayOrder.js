...
import { getOrderById, getUserTaxInfo, createOrUpdateTakeawayOrder } from 'api/orderService';
...
const TakeawayOrder = () => {
  ...
  const [customerInfo, setCustomerInfo] = useState({ name: '', phone: '', comment: '' });
  ...
  const initialStateRef = useRef({ orderItems: [], customerInfo: { name: '', phone: '', comment: '' } });
  ...
  async function fetchOrderDetails() {
    try {
      ...
      const custInfo = { name: order.customer_name || '', phone: order.customer_phone || '', comment: order.comment || '' };
      setOrderItems(items);
  ...
  const handleSaveOrder = async (status = 'Save') => {
    ...
    try {
      ...
      const payload = { orderInfo: { ...orderData, order_id: orderId }, customerInfo: { name: customerInfo.name, phone: customerInfo.phone } };
      const token = localStorage.getItem('token');
  ...
  return (
    <>
            ...
            <Card.Body className="p-0 p-lg-3">
              <div className="d-none d-lg-block">
                <CustomerInfoForm
                  ...
                  requiredFields={{ name: false, phone: false }}
                />
      ...
      <PaymentModal
        ...
        orderItems={orderItems} customerInfo={customerInfo} orderType="Takeaway" orderId={orderId} orderNo={orderNo}
      />
