...
export default function Contact() {
  const { settings, restaurantCode } = useRestaurant();
  const { user } = useAuth();
  ...
  const CONTACT_INFO = [
    ...
    { Icon: Phone, label: 'Contact', value: settings?.contact_phone || '+44 20 7946 0823' },
  ...
  const onSubmit = async (data) => {
    try {
      const response = await fetch(`${API_URL}/website/contact/${restaurantCode}`, {
        method: 'POST',
        headers: {
      ...
      <div className="container-lg" style={{ maxWidth: '1140px' }}>
        ...
        <div data-reveal="bottom" className="text-center mb-5">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <p className="section-subtitle mb-2">{settings?.contact_subtitle || 'Get in Touch'}</p>
            <h1 className="section-title text-white">
              {settings?.contact_title ? (
                (() => {
                  const words = settings.contact_title.split(' ');
                  if (words.length > 1) {
            ...
            <p className="text-white-60 mt-3 mx-auto" style={{ maxWidth: '36rem' }}>
              {settings?.contact_details || "Whether you're booking a table, asking about our menu, or just want to say hello — reach out and we'll respond promptly."}
            </p>
          </motion.div>
        </div>
            ...
            <div className="glass rounded-4 p-4 p-md-5">
              {sent ? (
                ...
                <form onSubmit={handleSubmit(onSubmit)} noValidate className="d-flex flex-column gap-3 h-100">
                  ...
                  <button type="submit" disabled={isSubmitting} className="btn-primary w-100 justify-content-center mt-2" style={{ opacity: isSubmitting ? 0.6 : 1 }}>
                    {isSubmitting ? (
                      <>
