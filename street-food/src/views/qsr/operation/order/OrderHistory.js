  ...
  const exportToPDF = async () => {
    try {
      ...
      if (exportFilters.fromDate || exportFilters.toDate)
        activeFilters.push({
          label: 'Date Period',
          val: `${exportFilters.fromDate ? format(new Date(exportFilters.fromDate), 'dd MMM yyyy') : 'All'} to ${exportFilters.toDate ? format(new Date(exportFilters.toDate), 'dd MMM yyyy') : 'All'
            }`,
        });
    ...
    <>
      <style>{`
        ...
        @media (max-width: 767px) {
          .mobile-order-history-container {
      ...
      `}</style>
      <div className="container-fluid qsr-page-container mobile-order-history-container">
                ...
                <Row className="g-3">
                  ...
                  <Col xs="12" sm="6">
                    ...
                    <div className="position-relative w-100">
                      <div
                        className="form-control rounded-pill px-3 border-0 shadow-sm d-flex align-items-center justify-content-between text-dark"
                        style={{ height: '44px', fontSize: '14px', backgroundColor: '#fff', pointerEvents: 'none' }}
                      >
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered className="modal-glass">
        ...
        <Modal.Body className="px-4 pt-4">
          <p className="text-muted mb-4">Are you sure you want to permanently delete this saved order? This action cannot be undone.</p>
          <div className="d-flex justify-content-end gap-3 mt-4">
            ...
            <Button variant="danger" onClick={handleDeleteOrder} className="px-4 rounded-pill fw-bold shadow-sm" disabled={loading}>
      ...
      <Modal show={showExportModal} onHide={() => !exporting && setShowExportModal(false)} size="lg" centered className="modal-glass modal-export-responsive">
        ...
        <Modal.Body className="px-4 pt-4">
          {!exporting ? (
            <>
              <p className="text-muted mb-4 export-subtitle">Select your preferred format and filters</p>
