    ...
    return (
      <div ref={ref} style={style} className={classNames('wide notification-dropdown scroll-out', className)} aria-labelledby={labeledBy}>
        ...
        <div className="border-top pt-2 text-center" style={{ margin: '0 -1.5rem' }}>
          <NavLink to="/inquiries" className="d-block py-1 text-primary font-weight-bold" style={{ fontSize: '0.8rem' }}>
...
const Notifications = () => {
  ...
  console.log("Notifications.js items from Redux:", items);
  ...
  const formatNotification = (item) => {
    ...
    if (item.type === 'new_inventory_request') {
      ...
      detail = `New inventory request from Manager${itemsList}!`;
      link = '/operations/requested-inventory';
      icon = 'boxes';
