...
const StockManagement = () => {
  ...
  const description = 'Monitor current stock, configure alert thresholds, and log wastage.';
  const breadcrumbs = [
    { to: '', text: 'Home' },
  ...
  const handleWastageSubmit = async (e) => {
    ...
    try {
      setIsSubmitting(true);
      await logWastage({ item_name: wastageItem._id, unit: wastageItem.unit, quantity: wastageQty, wastage_type: wastageType, reason: wastageReason });
    ...
    <>
      ...
      {lowStockCount > 0 && (
        <Alert variant="warning" className="d-flex align-items-center gap-2 mb-3">
          ...
          <strong>{lowStockCount} item{lowStockCount > 1 ? 's are' : ' is'} below minimum stock threshold!</strong>
        </Alert>
      )}
      ...
      <Row>
        <Col sm="12">
          {loading ? (
            <div className="d-flex justify-content-center align-items-center mb-5 mt-5"><Spinner animation="border" variant="primary" /></div>
          ) : stockData.length === 0 ? (
                        ...
                        <tr key={index} className={isBelowThreshold ? 'table-danger' : ''}>
                          ...
                          <td className="align-middle">
                            <span className={item.totalStock <= 0 ? 'text-danger fw-bold' : isBelowThreshold ? 'text-warning fw-bold' : 'text-success fw-bold'}>
                              {item.totalStock}
                            </span>{' '}<span className="text-muted text-small">{item.unit}</span>
                          </td>
                          <td className="align-middle">
                            {item.low_stock_threshold > 0 ? (
                              <Badge bg={isBelowThreshold ? 'danger' : 'light'} text={isBelowThreshold ? undefined : 'dark'}>Min: {item.low_stock_threshold}</Badge>
                            ) : <span className="text-muted text-small">Not set</span>}
                          </td>
                          <td className="align-middle"><span className="text-muted text-small">{item.tracking_level || 'auto'}</span></td>
                          <td className="text-end align-middle pe-3">
                            <div className="d-flex gap-1 justify-content-end">
                              <Button size="sm" variant="outline-danger" title="Log Wastage" onClick={() => openWastageModal(item)}><CsLineIcons icon="bin" size="13" /></Button>
                              <Button size="sm" variant="outline-secondary" title="Settings" onClick={() => openThresholdModal(item)}><CsLineIcons icon="gear" size="13" /></Button>
                              <Button size="sm" variant="outline-warning" disabled={item.totalStock <= 0} onClick={() => { setSelectedItem(item); setShowUseModal(true); }}>Mark Used</Button>
                            </div>
                          </td>
                        </tr>
                      );
        ...
        <Modal.Body>
          {selectedItem && (
            <Form onSubmit={handleUseSubmit}>
              ...
              <div className="mb-3"><strong>Available:</strong> {selectedItem.totalStock} {selectedItem.unit}</div>
              <Form.Group className="mb-3">
                <Form.Label>Quantity ({selectedItem.unit})</Form.Label>
              ...
              <div className="text-end">
                ...
                <Button variant="primary" type="submit" disabled={isSubmitting}>{isSubmitting ? <Spinner animation="border" size="sm" /> : 'Confirm Deduction'}</Button>
              </div>
            </Form>
          )}
        </Modal.Body>
          ...
          {wastageItem && (
            <Form onSubmit={handleWastageSubmit}>
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control as="textarea" rows={2} value={wastageReason} onChange={(e) => setWastageReason(e.target.value)} />
              </Form.Group>
              <div className="text-end">
                <Button variant="outline-secondary" className="me-2" onClick={() => setShowWastageModal(false)} disabled={isSubmitting}>Cancel</Button>
                <Button variant="danger" type="submit" disabled={isSubmitting}>{isSubmitting ? <Spinner animation="border" size="sm" /> : 'Log Wastage'}</Button>
              </div>
            </Form>
          )}
        </Modal.Body>
      ...
      <Modal show={showThresholdModal} onHide={() => !isSubmitting && setShowThresholdModal(false)} centered>
        ...
        <Modal.Body>
          <Form.Group className="mb-4">
            ...
            <Form.Text className="d-block text-muted mb-2">Warning badge when stock drops below this. Set 0 to disable.</Form.Text>
          ...
          <Form.Group>
            ...
            {TRACKING_LEVELS.map((t) => (
              <Form.Check key={t.value} type="radio" id={`adm-track-${t.value}`} name="adm_tracking_level" label={<>{t.label} <span className="text-muted small">ΓÇö {t.desc}</span></>} value={t.value} checked={trackingLevel === t.value} onChange={(e) => setTrackingLevel(e.target.value)} className="mb-2" />
            ))}
          </Form.Group>
        </Modal.Body>
        <Modal.Footer>
          <Button variant="outline-secondary" onClick={() => setShowThresholdModal(false)} disabled={isSubmitting}>Cancel</Button>
          <Button variant="primary" onClick={handleThresholdSave} disabled={isSubmitting}>{isSubmitting ? <Spinner animation="border" size="sm" /> : 'Save Settings'}</Button>
        </Modal.Footer>
      </Modal>
    </>
  );
