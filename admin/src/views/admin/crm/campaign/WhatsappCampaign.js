...
const WhatsappCampaign = () => {
  ...
  const buildMessage = (customer = null) => {
    ...
    if (customer) {
      body = body
        .replace(/\{\{customer_name\}\}/g, (customer.profile?.name || customer.name) || 'Valued Customer')
                  ...
                  customers.map((customer, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Avatar */}
                        <div className="picker-avatar">{getInitials((customer.profile?.name || customer.name))}</div>

                        {/* Info */}
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div className="picker-name">{(customer.profile?.name || customer.name) || 'Walk-in'}</div>
            ...
            <Modal.Body className="pt-2 pb-3 px-4">
              ...
              {currentCustomer && (
                <div className="p-3 rounded-3 text-center" style={{ background: 'linear-gradient(135deg,#f0fdf4,#dcfce7)', border: '1px solid #bbf7d0' }}>
                  ...
                  >
                    {getInitials((currentCustomer.profile?.name || currentCustomer.name))}
                  </div>
                  <div style={{ fontWeight: 700, color: '#1e293b', fontSize: '1rem' }}>{(currentCustomer.profile?.name || currentCustomer.name) || 'Walk-in'}</div>
