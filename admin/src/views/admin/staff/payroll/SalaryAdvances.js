...
const SalaryAdvances = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [advRes, staffRes] = await Promise.all([getSalaryAdvances(statusFilter), getStaffList()]);
      ...
      case 'active':
        return (
          <Badge
            ...
            style={{ backgroundColor: 'rgba(30, 168, 231, 0.08)', color: '#1ea8e7', border: '1px solid rgba(30, 168, 231, 0.15)' }}
          >
            Active
          </Badge>
        );
  ...
  return (
    <div className="container-fluid qsr-page-container">
              ...
              <tbody>
                {advances.map((adv) => (
                  <tr key={adv._id}>
                    ...
                    <td className="align-middle fw-medium">{adv.given_date ? format(new Date(adv.given_date), 'dd MMM yyyy') : '—'}</td>
                    ...
                    <td className="align-middle">
                      ...
                      {adv.recovery_mode === 'installments' && (
                        <div className="small text-muted fw-medium">
                          {adv.installments} EMIs of ₹{adv.installment_amount?.toFixed(2)}
                        </div>
                      )}
                    </td>
                    <td className="align-middle">
                    ...
                    <td className="text-end pe-4 align-middle">
                      {adv.status === 'active' && (
                        <Button variant="none" className="custom-btn-danger-outline" onClick={() => handleCancelAdvance(adv._id)}>
                          Cancel Deds
                        </Button>
                      )}
                    </td>
                      ...
                      <div className="mobile-advance-grid mb-3">
                        ...
                        <div className="grid-box">
                          ...
                          <span className="text-muted" style={{ fontSize: '0.6rem' }}>
                            Bal: ₹{adv.amount - adv.recovered_amount}
                          </span>
                        </div>
                      </div>
      ...
      <Modal show={showModal} onHide={() => !submitting && setShowModal(false)} centered size="lg" className="advance-modal">
        <Form onSubmit={handleSubmit}>
          ...
          <Modal.Body>
            <Row className="g-3">
              ...
              <Col md="6" className="mb-2">
                <Form.Group>
                  <Form.Label className="small fw-bold text-muted text-uppercase ms-1 mb-2">
                    Date Given <span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Control
                    type="date"
                    required
                ...
                <Col md="6" className="mb-2">
                  <Form.Group>
                    ...
                    {form.amount && form.installments > 0 && (
                      <div className="mt-2 text-success fw-bold small ms-1">Estimated EMI: ₹{(form.amount / form.installments).toFixed(2)} / month</div>
                    )}
                  </Form.Group>
                </Col>
              )}
          ...
          <Modal.Footer>
            ...
            <Button variant="none" className="custom-btn-primary-outline px-4" type="submit" disabled={submitting}>
