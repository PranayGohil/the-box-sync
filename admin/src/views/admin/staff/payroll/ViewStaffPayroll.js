...
import { Row, Col, Card, Button, Alert, Spinner, Badge, Modal, Form, ProgressBar, Table } from 'react-bootstrap';
...
export default function ViewStaffPayroll() {
  ...
  const downloadSalarySlip = (p) => {
    try {
      ...
      doc.setFontSize(9);
      doc.text(
        `Total Days: ${p.working_days_in_month}   |   Paid Days: ${p.leave_summary?.total_paid_days || p.present_days}   |   LWP: ${
          p.leave_summary?.lwp_days || p.absent_days
        }`,
        105,
        74,
        { align: 'center' }
    ...
    <div className="container-fluid qsr-page-container">
      ...
      <div className="qsr-page-title-container">
        <Row className="g-3 align-items-center">
          <Col md={7}>
            <h1 className="qsr-page-title">{staffData ? `${staffData.f_name} ${staffData.l_name}'s Ledger` : main_title}</h1>
            <BreadcrumbList items={breadcrumbs} />
          </Col>
          <Col md={5} className="d-flex justify-content-md-end align-items-center gap-3">
            <Button
              className="custom-btn-outline px-4 rounded-pill shadow-sm d-flex align-items-center"
              style={{ height: '40px' }}
      ...
      {error && (
        <Alert variant="danger" className="glass-card border-0 mb-4 shadow-sm">
          {error}
        </Alert>
      )}
      ...
      <Row className="g-4 mb-5">
        {[
          { label: 'Fiscal Months', value: careerStats.total_months || 0, icon: 'calendar', bg: 'soft-primary', color: 'primary' },
          {
            ...
            value: `₹${(careerStats.total_earned || 0).toLocaleString('en-IN')}`,
            icon: 'dollar',
            bg: 'soft-success',
            color: 'success',
          },
                    ...
                    <tr key={p._id}>
                      ...
                      <td className="text-end">
                        ...
                        <div className="small fw-bold text-danger">
                          Ded: -₹{((p.deduction_breakdown?.total_statutory || 0) + (p.deductions || 0)).toLocaleString('en-IN')}
                        </div>
                      </td>
                      <td className="text-end">
                      ...
                      <td className="text-center">
                        <div className="d-flex justify-content-center gap-2">
                          <Button
                            ...
                            className="btn-icon btn-icon-only rounded-circle d-flex align-items-center justify-content-center"
      ...
      <Modal show={showExportModal} onHide={() => setShowExportModal(false)} centered className="rounded-4">
        ...
        <Modal.Body className="p-4">
          <div className="glass-card bg-light border-0 p-4 mb-3 text-center">
            ...
            <p className="text-muted mb-0">
              Generating report for <strong>{filteredPayroll.length}</strong> payroll records based on active filters.
            </p>
          </div>
        ...
        <Modal.Footer className="border-0 p-4 pt-0 d-flex gap-3">
          <Button className="custom-btn-outline rounded-pill shadow-sm flex-grow-1" style={{ height: '40px' }} onClick={() => setShowExportModal(false)}>
            Cancel
          </Button>
          <Button className="custom-btn-solid rounded-pill shadow-sm flex-grow-1" style={{ height: '40px' }} onClick={handleExportConfirm}>
            Generate Asset
          </Button>
        </Modal.Footer>
      </Modal>
      ...
      <Modal show={showDetailModal} onHide={() => setShowDetailModal(false)} centered size="lg" className="rounded-4">
                  ...
                  <Row className="text-center g-3">
                    ...
                    <Col xs={6} md={3} className="border-end">
                      ...
                      <div className="fw-bold text-success fs-5">
                        {selectedPayroll.present_days}P / <span className="text-danger">{selectedPayroll.absent_days}A</span>
                      </div>
                    </Col>
                    <Col xs={6} md={3}>
                ...
                <div className="glass-card border-0 p-4 h-100 bg-white shadow-sm border border-soft-success">
                  ...
                  <div className="vstack gap-2">
                    {selectedPayroll.earned_breakdown ? (
                      <>
                        {['basic', 'hra', 'conveyance', 'medical', 'special', 'other'].map(
                          (key) =>
              ...
              <Col md={6}>
                <div className="glass-card border-0 p-4 h-100 bg-white shadow-sm border border-soft-danger">
                  ...
                  <div className="mt-auto pt-3 border-top d-flex justify-content-between align-items-center" style={{ marginTop: 'auto' }}>
                    ...
                    <span className="h5 mb-0 fw-bold text-danger">
                      ₹
                      {(
                        (selectedPayroll.deduction_breakdown?.total_statutory || 0) +
                        (selectedPayroll.deductions || 0) +
                        (selectedPayroll.advance_deduction || 0)
                      ).toLocaleString('en-IN')}
                    </span>
                ...
                <div className="glass-card border-0 bg-primary p-4 shadow-lg text-white d-flex justify-content-between align-items-center">
                  ...
                  <div className="text-end">
                    {selectedPayroll.status === 'paid' ? (
                      <Badge bg="white" className="text-primary status-badge px-4 py-2">
                        DISBURSED
                      </Badge>
                    ) : (
                      <Badge bg="white" className="text-warning status-badge px-4 py-2">
                        AUTHORIZED
                      </Badge>
                    )}
                  </div>
        ...
        <Modal.Footer className="border-0 p-4 pt-0">
          <Button className="custom-btn-outline rounded-pill shadow-sm w-100" style={{ height: '40px' }} onClick={() => setShowDetailModal(false)}>
            Close View
          </Button>
        </Modal.Footer>
      </Modal>
