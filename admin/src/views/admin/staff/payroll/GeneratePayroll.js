...
import { Row, Col, Card, Button, Badge, Alert, Modal, Form, Spinner, Table } from 'react-bootstrap';
...
export default function GeneratePayroll() {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_API}/payroll/preview?${params}`, authHeader());
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${process.env.REACT_APP_API}/payroll/generate`, body, authHeader());
  ...
  return (
    <div className="container-fluid qsr-page-container">
          ...
          <Row className="g-4 align-items-end">
              ...
              <Col xs={12} md={3}>
                ...
                {staffLoading ? (
                  ...
                  <Select
                    ...
                    value={
                      selectedStaffId
                        ? staffList.filter((s) => s._id === selectedStaffId).map((s) => ({ value: s._id, label: `${s.f_name} ${s.l_name} (${s.staff_id})` }))[0]
                        : null
                    }
                    onChange={(opt) => {
                      setSelectedStaffId(opt ? opt.value : '');
                      setPreviewFetched(false);
                    }}
            ...
            {generateMode === 'single' ? (
              <Col xs={12} className="d-flex justify-content-end mt-4">
                <Button
                  className="custom-btn-primary-outline px-5 rounded-pill shadow-sm"
                  style={{ height: '40px' }}
                  onClick={handlePreview}
                  disabled={isPreviewing || !selectedStaffId}
                >
                  {isPreviewing ? (
                    <Spinner size="sm" />
                  ) : (
                    <>
                      <CsLineIcons icon="eye" size="18" className="me-2" /> Fetch Preview
                    </>
                  )}
                </Button>
              </Col>
              ...
              <Col xs={12} md={3}>
                <Button
                  className="custom-btn-primary-outline w-100 rounded-pill shadow-sm"
                  style={{ height: '40px' }}
                  onClick={handlePreview}
      ...
      {generateResult && (
        <Alert
          variant="success"
          className="glass-card border-0 mb-4 p-4 d-flex align-items-center justify-content-between flex-wrap gap-3 shadow-lg"
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <div>
              ...
              {generateResult.data?.errors?.length > 0 && (
                <small className="text-danger">Encountered {generateResult.data.errors.length} skipped records.</small>
              )}
            </div>
          </div>
          <Button
            className="custom-btn-primary-outline px-4 rounded-pill shadow-sm"
            style={{ height: '40px' }}
        ...
        <Card className="glass-card border-0 overflow-hidden mb-5">
          <Card.Header className="bg-transparent border-0 p-4 pb-3" style={{ borderBottom: '1px solid #f1f5f9 !important' }}>
            <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
              <div>
                ...
                <small className="text-muted fw-medium text-uppercase" style={{ letterSpacing: '0.05em' }}>
                  Review adjustments before finalizing disbursement
                </small>
              </div>
                        ...
                        <tr className="hide-on-mobile">
                          ...
                          <td className="text-center">
                            <div className="d-flex align-items-center justify-content-center gap-2">
                              ...
                              <Badge
                                bg={p.absent_days > 0 ? 'soft-danger' : 'soft-light'}
                                className={`${p.absent_days > 0 ? 'text-danger' : 'text-muted'} px-2 py-1 rounded-pill`}
                              >
                                {p.absent_days}A
                              </Badge>
                            </div>
                          ...
                          <td className="text-end text-success fw-bold">
                            ₹{(p.earned_breakdown?.total_gross || earned || 0).toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                          </td>
                          <td>
                            <Form.Control
                          ...
                          <td>
                            <Form.Control
                              className="input-adjustment mx-auto text-success"
                              type="number"
                              value={adj.bonus ?? 0}
                          ...
                          <td>
                            <div className="text-danger small fw-bold text-center mb-1">
                              Stat: ₹{(p.deduction_breakdown?.total_statutory || 0).toLocaleString('en-IN')}
                            </div>
                            <Form.Control
                              className="input-adjustment mx-auto text-danger"
                              type="number"
                              placeholder="Manual"
                          ...
                          <td className="text-end">
                            <div className={`h5 mb-0 fw-bold ${net < 0 ? 'text-danger' : 'text-primary'}`}>
                              ₹{net.toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                            </div>
                          </td>
                          <td className="text-center">
                            {p.already_exists ? (
                              <Badge
                                bg={p.existing_status === 'paid' ? 'success' : 'warning'}
                            ...
                            <div className="p-3 border-bottom">
                              ...
                              <Row className="g-3 mb-3">
                                <Col xs={6}>
                                  ...
                                  <div className="d-flex align-items-center gap-2">
                                    ...
                                    <Badge
                                      bg={p.absent_days > 0 ? 'soft-danger' : 'soft-light'}
                                      className={`${p.absent_days > 0 ? 'text-danger' : 'text-muted'} px-2 py-1 rounded-pill fw-bold`}
                              ...
                              <Row className="g-2 mb-3">
                                <Col xs={4}>
                                  ...
                                  <Form.Control
                                    className="input-adjustment px-2"
                                    type="number"
                                    step={0.5}
                                    value={adj.overtime_hours ?? 0}
                                    onChange={(e) => updateAdjustment(p.staff_id, 'overtime_hours', e.target.value)}
                                ...
                                <Col xs={4}>
                                  ...
                                  <Form.Control
                                    className="input-adjustment text-success px-2"
                                    type="number"
                                    value={adj.bonus ?? 0}
                                    onChange={(e) => updateAdjustment(p.staff_id, 'bonus', e.target.value)}
                                  />
                                ...
                                <Col xs={4}>
                                  ...
                                  <Form.Control
                                    className="input-adjustment text-danger px-2"
                                    type="number"
                                    value={adj.deductions ?? 0}
                                    onChange={(e) => updateAdjustment(p.staff_id, 'deductions', e.target.value)}
                                  />
                                  <div className="text-danger text-end mt-1 fw-bold" style={{ fontSize: '0.65rem' }}>
                                    Stat: ₹{(p.deduction_breakdown?.total_statutory || 0).toLocaleString('en-IN')}
                                  </div>
                                </Col>
                              </Row>
                              ...
                              <div className="p-3 bg-light rounded-3 mt-2 mb-2 border border-light shadow-sm">
                                ...
                                <div className="d-flex justify-content-between align-items-center pt-2">
                                  ...
                                  <div className={`h4 mb-0 fw-bold ${net < 0 ? 'text-danger' : 'text-primary'}`}>
                                    ₹{net.toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                                  </div>
                                </div>
                              </div>
          ...
          <Card.Footer className="bg-light border-0 p-4">
            <div className="d-flex justify-content-between align-items-center flex-wrap gap-4">
              ...
              <Button
                className="custom-btn-primary-outline px-5 rounded-pill shadow-sm d-flex align-items-center"
                style={{ height: '40px' }}
              ...
              >
                {isGenerating ? (
                  ...
                  <>
                    <CsLineIcons icon="check" className="me-2" size="18" /> Finalize & Generate Payroll
                  </>
                )}
              </Button>
            </div>
      ...
      <Modal show={showConfirmModal} onHide={() => setShowConfirmModal(false)} centered className="rounded-4">
        ...
        <Modal.Body className="p-4 pt-0">
          <p className="text-muted">
            You are initiating the payroll generation for{' '}
            <strong>
        ...
        <Modal.Footer className="border-0 p-4 pt-0 d-flex gap-3">
          <Button
            className="custom-btn-primary-outline flex-grow-1 rounded-pill shadow-sm"
            style={{ height: '40px' }}
            onClick={() => setShowConfirmModal(false)}
          >
            Back to Review
          </Button>
          <Button className="custom-btn-primary-outline flex-grow-1 rounded-pill shadow-sm" style={{ height: '40px' }} onClick={handleGenerate}>
            {isGenerating ? <Spinner size="sm" /> : 'Execute Run'}
          </Button>
        </Modal.Footer>
      </Modal>
    </div>
