          ...
          {loading ? (
            ...
            <Table responsive hover className="mb-0">
              ...
              <tbody>
                {holidays.map((h) => (
                  <tr key={h._id}>
                    <td className="ps-4 align-middle fw-semibold">{format(new Date(h.date), 'dd MMM yyyy, EEEE')}</td>
                    <td className="align-middle">{h.name}</td>
                    <td className="align-middle">
                      <Badge bg={h.holiday_type === 'national' ? 'primary' : h.holiday_type === 'public' ? 'info' : 'secondary'} className="text-capitalize">
                        {h.holiday_type}
                      </Badge>
                    </td>
                    <td className="align-middle">
