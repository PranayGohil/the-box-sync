...
const LeaveRequests = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [reqRes, balRes, polRes] = await Promise.all([getLeaveRequests(statusFilter), getLeaveBalances(currentYear), getLeavePolicy()]);
          ...
          <Tabs activeKey={activeTab} onSelect={(k) => setActiveTab(k)} className="mb-4">
            ...
            <Tab eventKey="balances" title="Staff Leave Balances">
              ...
              {loading ? (
                ...
                <div className="text-center py-5 text-muted">No balance records found. Initialize balances when adding staff.</div>
              ) : (
                    ...
                    {balances.map((b) =>
                      b.balances.map((lt, idx) => (
                        <tr key={`${b._id}-${idx}`}>
                          ...
                          <td className="align-middle">{leavePolicy[lt.leave_type_id]?.name || lt.leave_type_id}</td>
                          ...
                          <td className="text-center align-middle text-dark fw-bold">{lt.entitled + lt.carried_forward - lt.taken - lt.pending}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
