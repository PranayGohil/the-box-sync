...
import { Row, Col, Card, Button, Badge, Alert, Modal, Form, Spinner, Table, InputGroup } from 'react-bootstrap';
...
export default function ManagePayroll() {
  ...
  const fetchSummary = async () => {
    ...
    try {
      const res = await axios.get(`${process.env.REACT_APP_API}/payroll/summary/${month}/${year}`, authHeader());
  ...
  const handleMarkPaid = async (ids) => {
    ...
    try {
      const res = await axios.put(`${process.env.REACT_APP_API}/payroll/mark-paid`, { ids: Array.from(ids) }, authHeader());
  ...
  const handleMarkUnpaid = async (ids) => {
    setIsMarkingPaid(true);
    try {
      const res = await axios.put(`${process.env.REACT_APP_API}/payroll/mark-unpaid`, { ids: Array.from(ids) }, authHeader());
      toast.success(res.data.message);
      setSelectedIds(new Set());
      fetchSummary();
    } catch {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await axios.put(`${process.env.REACT_APP_API}/payroll/update/${editingPayroll._id}`, editForm, authHeader());
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      await axios.delete(`${process.env.REACT_APP_API}/payroll/delete/${deletingPayroll._id}`, authHeader());
    ...
    <div className="container-fluid qsr-page-container">
      ...
      <div className="qsr-page-title-container">
        <Row className="g-3 align-items-center">
          ...
          <Col xs="12" md="6" className="d-flex flex-wrap justify-content-md-end align-items-center gap-3">
            <Button
              className="custom-btn-primary-outline px-4 rounded-pill shadow-sm d-flex align-items-center"
              style={{ height: '40px' }}
              as={Link}
              to="/staff/payroll/generate"
            >
          ...
          {[
            {
              ...
              value: `₹${(totals.total_net_salary || 0).toLocaleString('en-IN', { minimumFractionDigits: 0 })}`,
              icon: 'dollar',
              color: 'primary',
            },
            {
              label: 'Total Paid',
              value: `₹${(totals.total_paid || 0).toLocaleString('en-IN', { minimumFractionDigits: 0 })}`,
              icon: 'check-circle',
              color: 'success',
            },
            {
              label: 'Pending Payment',
              value: `₹${(totals.total_unpaid || 0).toLocaleString('en-IN', { minimumFractionDigits: 0 })}`,
              icon: 'clock',
              color: 'warning',
            },
      ...
      {selectedIds.size > 0 && (
        <div className="bulk-bar d-flex align-items-center justify-content-between shadow-lg">
          <div className="d-flex align-items-center gap-3">
            <div className="sw-5 sh-5 rounded-circle bg-white text-primary d-flex align-items-center justify-content-center fw-bold">{selectedIds.size}</div>
            <div>
          ...
          <div className="d-flex gap-2">
            <Button
              variant="light"
              className="rounded-pill px-4 fw-bold text-success d-flex align-items-center gap-2"
            ...
            >
              {isMarkingPaid ? (
                ...
                <>
                  <CsLineIcons icon="check" size="18" /> Mark Paid
                </>
              )}
            </Button>
            <Button
              variant="light"
              className="rounded-pill px-4 fw-bold text-warning d-flex align-items-center gap-2"
          ...
          {loading ? (
            ...
            <div className="text-center py-5">
              <div className="py-5">
                ...
                <Button className="custom-btn-solid" as={Link} to="/staff/add">
                        ...
                        {payroll ? (
                          <>
                            <td className="text-center">
                              <div className="d-flex align-items-center justify-content-center gap-2">
                                <Badge bg="soft-success" className="text-success px-2 py-1 rounded-pill">
                                  {payroll.present_days}P
                                </Badge>
                                <Badge
                                  bg={payroll.absent_days > 0 ? 'soft-danger' : 'soft-light'}
                                  className={`${payroll.absent_days > 0 ? 'text-danger' : 'text-muted'} px-2 py-1 rounded-pill`}
                                >
                                  {payroll.absent_days}A
                                </Badge>
                              </div>
                            </td>
                            <td className="text-end fw-medium text-dark">₹{(staff.salary || payroll.base_salary || 0).toLocaleString('en-IN')}</td>
                            <td className="text-end text-success fw-bold">
                              ₹{(payroll.earned_breakdown?.total_gross || payroll.earned_salary || 0).toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                            </td>
                            <td className="text-end">
                              ...
                              <div className="small fw-bold text-danger">
                                Ded: -₹{((payroll.deduction_breakdown?.total_statutory || 0) + (payroll.deductions || 0)).toLocaleString('en-IN')}
                              </div>
                            </td>
                            <td className="text-end fw-bold text-primary h5 mb-0">
                              ₹{payroll.net_salary.toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                            </td>
                            <td className="text-center">
                          ...
                          <>
                            ...
                            <td className="text-center bg-light bg-opacity-10">
                              <Button
                                className="custom-btn-solid rounded-pill shadow-sm px-4"
                                style={{ height: '40px' }}
                              ...
                              <div>
                                {payroll ? (
                                  <Badge
                                    ...
                                    className="status-badge py-1 px-2"
                                  ...
                                  <Badge bg="soft-secondary" className="text-muted status-badge py-1 px-2" style={{ fontSize: '0.65rem' }}>
                                    UNGENERATED
                                  </Badge>
                                )}
                              </div>
                                ...
                                <Row className="g-3 mb-3">
                                  <Col xs={6}>
                                    ...
                                    <div className="d-flex align-items-center gap-2">
                                      <Badge bg="soft-success" className="text-success px-2 py-1 rounded-pill fw-bold" style={{ fontSize: '0.8rem' }}>
                                        {payroll.present_days}P
                                      </Badge>
                                      <Badge
                                        bg={payroll.absent_days > 0 ? 'soft-danger' : 'soft-light'}
                                        className={`${payroll.absent_days > 0 ? 'text-danger' : 'text-muted'} px-2 py-1 rounded-pill fw-bold`}
                                ...
                                <div className="p-3 bg-light rounded-3 mt-2 mb-4 border border-light shadow-sm">
                                  ...
                                  >
                                    ...
                                    <div className="text-success fw-bold">
                                      ₹
                                      {(payroll.earned_breakdown?.total_gross || payroll.earned_salary || 0).toLocaleString('en-IN', {
          ...
          <Card.Footer className="bg-light border-0 p-4">
            <div className="d-flex flex-wrap gap-4 align-items-center justify-content-between">
              ...
              <div className="h4 mb-0 fw-bold text-dark">
                Total Disbursement:{' '}
                <span className="text-primary">₹{(totals.total_net_salary || 0).toLocaleString('en-IN', { minimumFractionDigits: 0 })}</span>
              </div>
            </div>
          </Card.Footer>
        )}
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)} centered size="lg" className="rounded-4">
            ...
            <>
                ...
                <Row className="text-center g-3">
                  <Col xs={4}>
                    ...
                    <div className="fw-bold h5 mb-0 text-dark">
                      ₹{(editingPayroll.staff?.salary || editingPayroll.base_salary || 0).toLocaleString('en-IN')}
                    </div>
                  </Col>
                  <Col xs={4}>
              ...
              <Row className="g-4">
                ...
                <Col xs={12} md={6}>
                  ...
                  <Form.Control
                    className="rounded-3 border-0 bg-light py-2 px-3 text-success fw-bold"
                    type="number"
                ...
                <Col xs={12} md={6}>
                  ...
                  <Form.Control
                    className="rounded-3 border-0 bg-light py-2 px-3 text-danger fw-bold"
                    type="number"
                ...
                <Col xs={12}>
                  ...
                  <Form.Control
                    className="rounded-3 border-0 bg-light py-2 px-3"
                    type="text"
                    placeholder="e.g. Performance bonus or late fine"
        ...
        <Modal.Footer className="border-0 p-4 pt-0 d-flex gap-3">
          <Button className="custom-btn-primary-outline rounded-pill shadow-sm flex-grow-1" style={{ height: '40px' }} onClick={() => setShowEditModal(false)}>
            Discard Changes
          </Button>
          <Button
            className="custom-btn-primary-outline rounded-pill shadow-sm flex-grow-1"
            style={{ height: '40px' }}
          ...
          >
            {isSavingEdit ? (
              ...
              <>
                <CsLineIcons icon="save" className="me-2" size="18" /> Commit Updates
              </>
            )}
          </Button>
        </Modal.Footer>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered className="rounded-4">
        ...
        <Modal.Footer className="border-0 p-4 pt-0 d-flex gap-3">
          ...
          <Button
            variant="outline-danger"
            className="rounded-pill shadow-sm flex-grow-1 d-flex align-items-center justify-content-center gap-2"
          ...
          >
            {isDeleting ? (
              ...
              <>
                <CsLineIcons icon="bin" size="18" /> Confirm Delete
              </>
            )}
          </Button>
        </Modal.Footer>
